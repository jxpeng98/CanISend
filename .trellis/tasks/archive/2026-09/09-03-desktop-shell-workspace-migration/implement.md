...
- [x] Fetch and inspect current `origin/main`; preserve unrelated local branches and user work.
- [x] Create `fix/desktop-shell-workspace-migration` from current `origin/main`, set the Trellis
      branch/base/scope metadata, and commit the approved planning artifacts before product edits.
- [x] Confirm `origin/main...HEAD` contains no prior closeout-only or unrelated commits.
...
- [x] Update the existing shell contract test first for a one-line brand, no local-storage footer,
      sidebar-owned appearance actions, no redundant main toolbar, and a fixed notification region.
- [x] Remove the sidebar tagline, tighten its header, and replace the footer panel with the existing
      language, theme, and density actions.
- [x] Remove the redundant sticky main toolbar while retaining page headings, document titles,
      native window behavior, and the skip link.
- [x] Move concise local-storage/privacy copy into Settings beside version/about information.
- [x] Move App-wide error/success feedback into one dismissible fixed popup while preserving retry,
      result navigation, severity, live-region semantics, and state clearing.
- [x] Update English and Simplified Chinese keys without adding framework or implementation jargon.
...
- [x] Rebuild System diagnostics from the shared Card plus existing Accordion primitives, preserving
      progressive disclosure and all states/actions.
- [x] Update Playwright coverage for sidebar keyboard operation, persisted language/theme/density,
      non-layout-shifting popup feedback, dismissal/retry, diagnostics disclosure, and 200% Chinese
      layout.
- [x] Run the application-shell accessibility spec and inspect one bounded pair of rendered states:
      English/light/comfortable at 1280×820 and Chinese/dark/compact at 960×680 with 200% text.
- [x] Apply one batch of confirmed visual fixes, recheck once, then stop polishing.
...
- [x] Replace the repeated migration dispatch with the existing ordered SQL files executed under
      one immediate transaction; validate contiguous order and final schema version before commit.
- [x] Add one focused regression that injects a valid pending migration followed by a failing one,
      asserts exact pre-upgrade schema/history/version after rollback, then proves a corrected retry.
- [x] Retain the existing future-schema and incomplete-history no-mutation tests.
...
- [x] Run the existing v4 recovery round trip, malformed/legacy backup refusal,
      occupied-destination preservation, current-schema reopen, and Store migration tests.
- [x] Record exact commands/results and the honest no-schema-delta limitation in a body-free task
...
- [x] Run `trellis-check`, review the full diff for accidental churn, dead imports/styles, private
      data, generated output, and unsupported legacy migration exposure.
- [x] Run the Impeccable detector once over the changed UI targets and resolve real findings.
- [x] Commit the bounded product/test change with a Conventional Commit message.
- [x] Add one exact feature-freeze exception for every changed nonautomatic path, bound to that
...
- [x] Build the unpublished macOS Design Preview from the clean exception-bound commit and verify
