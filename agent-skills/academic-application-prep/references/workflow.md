...
4. Run `academic-prep extract-profile-evidence`.
5. Fetch jobs.ac.uk RSS leads with `academic-prep fetch-jobs-ac-uk`.
6. Create one job workspace with `academic-prep new-job-from-lead` or `academic-prep new-job`.
7. Paste or import the full selected advert into `job_advert.md`; RSS leads are not full adverts.
8. Run `academic-prep run --job jobs/<job-slug>`.
