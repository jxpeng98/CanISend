...
#let canisend_field(data, keys, fallback: none) = {
  ...
  if matches.len() > 0 { matches.first().value } else { fallback }
}

#let canisend_contacts(data) = {
  let contact-keys = (
    "email",
    "phone",
    "website",
  ...
  data.fields
    .filter(field => contact-keys.contains(field.key) and field.value != "")
    .map(field => (text: field.value,))
}

#let canisend_profile(data) = (
  name: canisend_field(data, ("candidate-name", "applicant-name", "name")),
  role: canisend_field(data, ("candidate-role", "current-role", "role", "position")),
  address: canisend_field(
    data,
    ("candidate-address", "applicant-address", "address"),
  ),
  contacts: canisend_contacts(data),
)

#let canisend_body(data, show-headings: false) = {
  for section-data in data.sections {
    if show-headings and section-data.heading != none {
      heading(level: 1, outlined: false, section-data.heading)
    }
    section-data.body
    parbreak()
  }
...
#let canisend_render_document(data) = {
  ...
  let profile = canisend_profile(data)
  if data.kind == "cover-letter" {
    let explicit-greeting = canisend_field(data, ("salutation", "greeting"))
    let addressee = canisend_field(data, ("addressee", "recipient-name"))
    let greeting = if explicit-greeting != none {
      explicit-greeting
    } else if addressee != none {
      "Dear " + addressee + ","
    } else {
    ...
    coverletter(
      ...
      recipient: (
        greeting: greeting,
        subject: data.title,
        date: canisend_field(data, ("date", "letter-date"), fallback: data.generated-date),
        name: canisend_field(data, ("recipient-name",)),
        role: canisend_field(data, ("recipient-role", "recipient-position")),
        department: canisend_field(data, ("recipient-department", "department")),
        institution: canisend_field(data, ("recipient-institution", "institution")),
        address: canisend_field(data, ("recipient-address",)),
        postcode: canisend_field(data, ("recipient-postcode", "postcode")),
      ),
      closing: (
        salutation: canisend_field(data, ("closing", "sign-off"), fallback: "Sincerely,"),
      ),
    )[
      #canisend_body(data)
    ]
    ...
    statement(
      ...
      #canisend_body(data, show-headings: true)
    ]
  }
}
