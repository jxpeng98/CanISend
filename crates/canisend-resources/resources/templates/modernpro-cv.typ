...
#let canisend_profile(data, name-fallback: none) = (
  name: canisend_field(
    data,
    ("candidate-name", "applicant-name", "name"),
...
#let canisend_render_document(data) = {
  ...
  cv-single(
    profile: canisend_profile(data, name-fallback: data.title),
    preset: "default",
    font-type: "Libertinus Serif",
    continue-header: "true",
    lastupdated: "false",
    pagecount: "true",
    date: data.generated-date,
  )[
    #for section-data in data.sections {
      if section-data.heading != none {
        section(section-data.heading)
      }
      descript(section-data.body)
      sectionsep
    }
  ]
}
