...
const APPLICATION_PLANS_MIGRATION: &str = include_str!("../migrations/0008_application_plans.sql");
...
mod tests {
    ...
    fn version_one_workspace_upgrades_transactionally_to_current_schema() {
        ...
        let application_plan_table: i64 = database
            .connection
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master
                 WHERE type = 'table' AND name = 'application_plan_heads'",
                [],
                |row| row.get(0),
            )
        ...
        let document_heads_table: i64 = database
            ...
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master
                 WHERE type = 'table' AND name = 'document_heads'",
                [],
                |row| row.get(0),
            )
            .expect("document heads table");
        assert_eq!(document_heads_table, 1);
        let review_heads_table: i64 = database
            .connection
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master
                 WHERE type = 'table' AND name = 'review_heads'",
                [],
                |row| row.get(0),
            )
            .expect("review heads table");
        assert_eq!(review_heads_table, 1);
        let package_heads_table: i64 = database
            .connection
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master
                 WHERE type = 'table' AND name = 'package_heads'",
                [],
                |row| row.get(0),
            )
