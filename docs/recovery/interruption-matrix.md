...
| Legacy compatibility after Workspace v3 activation | Agent v2 or `job` requests a generic, digest-mismatched, unmapped, or write-intent operation | Exact academic reads are admitted; every unsupported combination returns canonical v3 remediation with `workspace_mutated: false`; migrated legacy rows and v3 authority remain unchanged | `compatibility::tests::generic_pack_context_fails_closed_with_canonical_remediation`; `compatibility::tests::exact_academic_pack_is_required_not_only_the_pack_id`; `compatibility::tests::migrated_academic_reads_pass_and_legacy_writes_do_not_mutate` |
...
| Application Pack migration stale preview | The Application head or managed projection set changes after review | Commit rejects the old preview digest and leaves head, ledger, audit, and projections unchanged | `pack_migration_v3::tests::stale_preview_and_ledger_failure_are_atomic_and_retryable` |
| Application Pack migration ledger failure | A bounded local trigger aborts the immutable ledger insert | The transaction rolls back every entity revision, head binding, dependency, and audit write; exact retry succeeds after the fixture is removed | `pack_migration_v3::tests::stale_preview_and_ledger_failure_are_atomic_and_retryable` |
| Pack migration supersedes projections | A current exact projection exists before the Pack version changes | Old rows remain immutable/superseded, ordinary repair does not restore them, and the migrated revision can publish a current projection set | `pack_migration_v3::tests::label_only_upgrade_rebinds_without_invalidating_outputs_and_supersedes_old_projections` |
