...
| T20 | A malformed MCP request, oversized result, forged workspace scope, unapproved write, replayed preview, or stale job/task state bypasses the CanISend control plane | The version-matched CLI owns a fixed `mcp serve` entry point; thirteen deterministic tools delegate to the application facade; write annotations are explicit; job intake and task completion use capped in-memory single-use tokens; consent, job revisions, leases, input revisions/hashes, schema, and source spans are revalidated; unknown fields are denied; IDs and serialized output are bounded; routine models exclude bodies | raw JSON-RPC negotiation/list/call fixtures; guarded annotation assertions; job intake preview/commit/replay protocol regression; task prepare/input consent regression; private sentinel, stale revision, and no-preview-mutation tests |
