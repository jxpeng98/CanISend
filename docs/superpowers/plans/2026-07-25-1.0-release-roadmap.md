...
defines the App-first Codex App Server/MCP Workbench, Tauri, traceability, and post-MVP ACP
direction. Accepted ADRs remain authoritative for product and architecture decisions.
...
Deliverables, collaborates through embedded Codex or a qualified external Agent using explicit
...
| Codex and external Agent hosts | Codex remains the required external release host; the approved embedded MVP adds Codex App Server while v4 Skills/MCP preserve App-closed Codex, Claude Code, and Claude Desktop paths | Embedded support is claimed only after R0-R4 qualification; generated Claude compatibility and real-host observations keep their existing evidence status |
...
|---|---|---|
| Private Beta.2 source readiness is incomplete | P0 before Beta.2 source preparation | The supported CLI, Skills, MCP, and Workspace journey plus exact sequential-Beta governance must pass under the active freeze |
| The Agent screen can retain a retired Job scope after the current Workspace has no legacy selection | P0 release blocker in M4-BETA2-001 | Runtime-catalog loading can enter the intentional Workspace v4 legacy refusal even though Agent v4 handoff itself is clean |
| The App-first Codex client and simplified Workbench are not implemented | Planned 1.0 product work | ADR-RN-0022 requires a direct App Server slice, guarded MCP loop, exact managed-file history, and qualification before embedded support is claimed |
| Invited target-user evidence is absent in Issue #70 | P0 before RC | Provider dogfood and zero public issues do not prove that users can complete the mixed-Pack workflow; the cohort runs on public Beta.1 |
...
### 3.3 Approved App-first delivery sequence

ADR-RN-0022 approves the product and architecture direction below. The clean-v4 screen correction
is part of the existing `M4-BETA2-001` release-blocker scope. Each later phase remains Planned until
its required GitHub work item and active-freeze disposition are recorded; this roadmap does not
pre-authorize a tag, release, support claim, or broad feature exception.

```text
R0 architecture/authority + clean-v4 blocker
  -> R1 direct Codex App Server vertical slice
     -> R2 guarded CanISend MCP + exact managed-file history
        -> R3 conversation-centered Workbench + right Inspector
           -> R4 MVP recovery, accessibility, and native qualification
              -> R5 generic ACP v1 + first Claude Agent qualification
              -> R6 legacy surface removal after measured parity
```

R0 keeps Rust/Tauri/Svelte and the current exact-byte PDF preview. R1 invokes an already-installed,
qualified `codex app-server --listen stdio://`, reuses Codex-managed authentication, and stores no
provider credential or transcript body. R2 keeps MCP as the Workspace/Application-bound product
tool boundary and correlates body-free session/turn references with operations, receipts, previews,
approval outcomes, revisions, audit events, and output digests. R3 reduces primary navigation to
Work, Library, and Settings; conversation occupies the center and Context, Evidence, Changes,
Consent, History, and Preview occupy a right Inspector/drawer.

Git continues to track source and release provenance only. Exact file history uses append-only
manifests and existing verified Blobs for files produced by CanISend's managed projection/export
pipeline; it never scans arbitrary Workspace files or creates a Workspace repository. R2 may lock
`similar` 3.2.0 for bounded Myers line hunks only after its recorded source, Apache-2.0 license,
advisory, and maximum-input checks are rerun. No Git subprocess or repository library is retained as
a fallback.

Electron and PDF.js remain behind the objective ADR-RN-0022 preview gate because exact PDF preview
already passed WKWebView, WebView2, and WebKitGTK qualification. R5 starts only after the Codex MVP;
it adds a generic ACP v1 stdio channel and first qualifies Claude Agent. No provider trait, ACP
Registry auto-install, or Harness native-session claim is introduced before a second real session
implementation passes the common lifecycle suite.
...
- Alpha.7 candidate work starts only after clean-v4 App, CLI, MCP, Codex, Claude Code, and Claude
  Desktop flows pass.
- Mixed-Application invited-user evidence starts on public Beta.1 and must complete before RC.1
  planning; synthetic maintainer dogfood contributes zero users.
...
qualification/evidence child beside it. `M3-ARCH-001` and `M3-ALPHA9-001` are Verified.
...
repository baseline while retaining the qualified Beta artifact source. `M4-BETA2-001` is the
current private policy/readiness task; `M3-EVID-005` / Issue #70 remains required before RC.1. When sources
disagree, correct the lower-authority projection before advancing the affected gate; historical
Alpha evidence remains immutable.
...
| M4 — Beta.1, freeze, and private Beta.2 | 2–3 working days plus native runners | M3 | Qualified public Beta baseline, committed freeze, and reviewed private Beta.2 source |
...
|---|---:|---|
| Post-Beta invited cohort | 5–8 users and at least 10 complete flows across both Packs | Find initialization, mixed-Application, intake, Skills, consent, recovery, and export blockers on public Beta.1 |
...
`M3-EVID-005` retains its public identifier but is not an M3 or Beta-entry gate. It runs on public
Beta.1 and is required by the M4 exit gate before RC.1 planning.
...
| M3-EVID-005 | P0 | Run the invited and cumulative cohort on public Beta.1 before RC.1 planning | Body-free evidence binds Beta.1, both Packs, 8–12 users, at least 20 flows, mixed Workspaces, three non-academic families, metrics, exclusions, and no-submission understanding |
...
**Entry:** The latest active Beta is qualified, feature freeze is active, and the required
post-Beta cohort evidence is complete.
...
| Real-provider host coverage | Exact Alpha.10 Codex CLI for both Packs; Claude resources remain contract-checked and real-host observations are non-blocking | No regression on the required Codex surface; retained observations remain truthful |
| Cumulative target users | Zero claimed; cohort starts on public Beta.1 | At least 8–12 represented in the frozen evidence |
| Completed supported end-to-end flows | Zero claimed; cohort starts on public Beta.1 | At least 20 represented in the frozen evidence |
| Unassisted supported-flow completion | Not a Beta-entry gate; denominator starts on public Beta.1 | At least 80%, no unresolved blocker pattern |
...
17. **Completed 2026-08-30:** activated feature freeze at exact protected repository baseline
    ...
    initial exceptions.
18. **Next:** merge `M4-BETA2-001`, prepare the private Beta.2 source through `M4-BETA2-002`, and
    keep Beta.2 qualification/publication separate.
19. Complete `M3-EVID-005` / Issue #70 before RC.1 planning.
20. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
