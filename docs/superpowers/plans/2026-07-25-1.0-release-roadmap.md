...
| LF-C04 | LF-C03 | Trusted windowless preview/confirmation/commit in the correct Broker; reject wrong scope, stale/replayed grants and restart reuse | Source implementation in progress; actual Host confirmation qualification remains LF-C05 |
| LF-C05 | LF-C04 | One Host completes source intake, Application preparation, candidate review, export/recovery; another local session resumes canonical state | Planned |
| LF-C06 | LF-C05 | Reuse existing Task/Review/Store for minimal atomic claims and bounded candidate handoff; separate coordination and content revisions | Planned |
...
| LF-C09 | LF-C05 | Optional additional Host adapters with individual capability and dual-Pack evidence | Optional |
...
LF-C01 and LF-C02 merged through PRs #226/#227. LF-C03 adds isolated resource and lifecycle
checks and repairs the v4 upgrade harness; its PR owns protected integration evidence. LF-C04
is the next bounded slice. LF-C10 owns exact native qualification and does not wait for collaboration
...
4. **Qualify:** inspect and accept protected Fast CI as the complete source owner. Run native
...
The process cleanup does not relax feature freeze: policy-bearing adapter/AGENTS/PRODUCT/spec
changes still need exact post-baseline exception records when committed. Do not broaden automatic
path exemptions, fabricate a future commit, or change a qualification ledger to hide missing evidence.
