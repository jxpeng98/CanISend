...
- Mixed-Application invited-user evidence is currently bound to public Beta.1. LF-C10 must
  explicitly update the existing validator before the formal cohort targets a qualified embedded
...
flowchart LR
    ...
    M4 --> APP["R2a/R2b: bound tools and exact history"]
    APP --> WORK["R3: complete Workbench"]
    WORK --> QUAL["R4: exact embedded Beta qualification"]
    QUAL --> COHORT["Issue 70 / APP-005: one exact-build cohort and validated evidence"]
...
| M4 — Beta baseline, embedded App, and validation | Re-estimate after R2a protocol proof; R0/R1 source already complete | M3 | Qualified embedded Beta, preserved Beta.1 history, freeze disposition, and exact-build user evidence |
...
pre-embedded scope; they are historical planning context, not an estimate for the newly approved
Workbench. Re-estimate remaining R2-R4 effort from the R2a protocol proof and first complete journey.
...
## 12. M4 — Beta baseline, embedded App, and feature freeze

**Objective:** Preserve the qualified Beta.1 baseline and public contract freeze while delivering
and qualifying the approved embedded App under exact exceptions before RC.
...
| Real-provider host coverage | Exact Alpha.10 Codex CLI for both Packs; Claude observations remain non-blocking | Required external Codex remains usable; exact embedded App Server version/capabilities and R4 build are separately qualified |
...
| Cumulative target users | Zero claimed at historical Beta.1 entry | At least 8, target 8–12, on the validator-approved embedded build |
| Completed supported end-to-end flows | Zero claimed at historical Beta.1 entry | At least 20 on the validator-approved embedded build |
| Unassisted supported-flow completion | Not a historical Beta-entry gate | At least 80% on the exact embedded build; no pooled old-UI denominator or unresolved blocker pattern |
