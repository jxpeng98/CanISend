...
**Status:** Active — authoritative; Alpha.8 is published and Beta.1 readiness is pending
...
from `35e7c822ea2f469ab726a31b5d08e622f6810c55`; later `main` changes still require a new exact
candidate before they may be described as published bytes.

**Current public checkpoint:** [`v1.0.0-alpha.8`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-alpha.8)
at `35e7c822ea2f469ab726a31b5d08e622f6810c55`, published 2026-08-10. The tag, public artifacts,
...
**Next intended checkpoint:** `v1.0.0-beta.1` may freeze the published Alpha.8 Workspace v4,
...
| Post-Alpha.8 source | Contains release-evidence reconciliation only; no later commit or PR is published Alpha.8 bytes and every replacement checkpoint requires exact qualification |
...
bounded MCP host. Remaining M3 work is invited-user evidence, post-publication blocker disposition,
and Beta-entry readiness against that exact tested checkpoint. It must not create a parallel
engine, a second approval system, or relabel later source as published Alpha.8.
...
qualification/evidence child beside it. After `M3-ALPHA8-001` reconciliation merges, the current
delivery task is `M3-EVID-005` / Issue #70. Create `M4-READY-001` / Issue #71 only after the M3
exit gate passes. When sources disagree, correct the lower-authority projection before advancing
the affected gate.
...
**Entry:** M2 and M3 are Verified. Public Alpha.8 has no unresolved P0/P1 blocker and is the latest
qualified Alpha.

| ID | Priority | Deliverable | Verification |
|---|---|---|---|
| M4-READY-001 | P0 | Refresh Beta readiness against Alpha.8 and reviewed mixed-Application Agent/user evidence | Snapshot is no older than 24 hours, binds Workspace v4, Agent v4, Skills and both Pack digests, is body-free, and zero issues is not the only blocker evidence |
| M4-FREEZE-001 | P0 | Run `release freeze-candidate` for Agent v4, Workspace v4, Skills manifest/resources, Pack v1, both built-in Packs, schemas, operation registry, exit codes, and bundle layout | Baseline digests match the qualified Alpha.8 surface; no Alpha.6, v2, or v3-only baseline can pass |
...
| Start Beta transition | M2 + M3 complete against Alpha.8; readiness refreshed within 24 hours; no P0/P1 blockers |
