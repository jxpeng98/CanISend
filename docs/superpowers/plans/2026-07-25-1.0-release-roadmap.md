...
checkpoint at `beta-qualifying`, feature freeze is active, and private Beta.2 native candidate
verification is the current release-engineering checkpoint
...
**Next intended checkpoint:** `v1.0.0-beta.2` is the active private source checkpoint under the
same feature freeze. The latest qualified public checkpoint remains [`v1.0.0-beta.1`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-beta.1),
whose artifact source is `6e1397b79031cad54e794ccdc9edca2153f23b3e`; the separate protected
repository baseline is `acf25dc483643ca9be0210320775708da116b715`. One nonpublishing native
candidate is the next authorized release-engineering action. No Beta.2 tag, release, package, or
qualification is authorized here, and the consented body-free cohort remains required before RC.1
planning.
...
Private Beta.2 policy/readiness and source preparation are also complete. One nonpublishing native
candidate must now prove the exact Beta.2 source across the existing release matrix; invited-user
evidence remains required before RC.1. None of these tasks may create a parallel engine, a second
approval system, modify an external package index, or rewrite historical Alpha evidence.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Why it blocks progress |
|---|---|---|
| Private Beta.2 native candidate evidence is incomplete | P0 before any Beta.2 release decision | The exact prepared source must pass the existing native, integrity, signing-evidence, and provenance matrix without creating a tag or release |
...
`M4-BETA2-002` are Verified through protected PRs #219 and #220; `M4-BETA2-003` is the current
private native-candidate task. `M3-EVID-005` / Issue #70 remains required before RC.1. When sources
...
`M4-BETA2-003` now owns one nonpublishing native candidate and independent inspection. None of
these tasks authorizes Beta.2 tagging, publication, or qualification.
...
- [ ] Exact private Beta.2 native candidate bytes pass independent integrity and provenance checks
...
19. **Next:** build and independently inspect one exact nonpublishing Beta.2 native candidate
    through `M4-BETA2-003`.
20. Complete `M3-EVID-005` / Issue #70 before RC.1 planning.
...
- [ ] One exact private Beta.2 native candidate passes independent integrity and provenance checks
