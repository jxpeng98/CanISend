...
| Rust 1.97 source alignment awaits an exact current-commit CI result | P0 | Local source checks cannot replace the required continuously tested evidence |
...
| M0-REL-001 | [Sequential Alpha transition implementation record](../../notes/rust-native/2026-08-03-m0-sequential-alpha-transition.md), stage-transition policy/runbook, transactional writer, and 27-file Alpha.6 dry-run | Source implementation and dry-run are complete; link the work item, inspect the committed plan, and obtain explicit authority before applying Alpha.6 or starting a candidate |
| M0-GOV-001 | [GitHub governance audit](../../notes/rust-native/2026-08-03-m0-github-governance-audit.md) confirms the required milestones, roadmap labels, and Issues do not yet exist | Obtain explicit repository-governance authority, create the reviewed desired state, and link every open critical-path ID |
| M0-GOV-002 | The same audit confirms `main` returns GitHub's `Branch not protected` response and the repository has no rulesets | Obtain explicit authority and apply reviewed main/tag rules without requiring an unavailable second reviewer |
...
| Task | Repository evidence | Remaining state boundary |
|---|---|---|
| M1F-PACK-001 | [Workflow Pack v1 contract](../../contracts/workflow-pack-v1.md), [generic starter Pack contract](../../contracts/generic-application-workflow-pack-v1.md), [GF4 implementation record](../../notes/rust-native/2026-08-03-gf4-generic-starter-pack.md), and [synthetic examples record](../../notes/rust-native/2026-08-03-gf4-synthetic-examples.md) | Two built-in Packs now pass the same exact resource/digest/runtime/capability registry with disjoint stage and Deliverable catalogs; generic CLI/MCP/Agent/desktop flows, four offline domain families, and the two-Pack minimum semantic matrix are implemented. External installation, native qualification, and work-item linkage remain before this item can be marked Verified |
| M1F-DELIV-001 | [Canonical Application flow v3 contract](../../contracts/application-flow-v3.md), [GF4 flow implementation record](../../notes/rust-native/2026-08-03-gf4-canonical-generic-flow.md), and [generic starter Pack contract](../../contracts/generic-application-workflow-pack-v1.md) | The generic Pack completes plan, compose, review, managed package projection, neutral template render, and consented export with two custom Deliverable kinds; academic canonical-v3 parity, shared adapter registration, native qualification, and work-item linkage remain before this item can be marked Verified |
...
| M1F-SURFACE-001 | [Workflow Pack presentation contract](../../contracts/workflow-pack-presentation-v1.md), [GF3 desktop implementation record](../../notes/rust-native/2026-08-03-gf3-pack-backed-ui.md), [GF4 generic CLI/desktop record](../../notes/rust-native/2026-08-03-gf4-generic-cli-desktop.md), and [Agent v3/MCP contract](../../contracts/agent-v3-mcp.md) | App facade, CLI, Tauri, Agent v3/MCP, and desktop expose exact Pack selection plus generic create/resume/plan/compose/review/approve/export with English/Chinese Pack labels and no fixed academic Deliverable branch; canonical operation and semantic registries are source-gated. Direct v3 source adapters, native qualification, and work-item linkage remain before this item can be marked Verified |
...
bindings.
Exact remote CI evidence is still required before the MSRV evidence checkbox is complete, and the
P1 lightweight Linux/Windows and Chrome expansion is now implemented in source as recorded by the
...
keyboard/accessibility/key-visual job. Exact committed GitHub runner results remain required before
the two M1A CI checkboxes can close.
...
reachable. Exact committed workflow evidence and independent review remain before the M1A checkbox
can close.
...
stores, predictable adapter tokens, or missing expiry contracts from returning. The remaining M1A
gates remain open.
...
Current-HEAD local source evidence is recorded in the
...
v2 compatibility authority. This is candidate-readiness evidence only: current HEAD is still
Alpha.5, unpushed, and not an exact Alpha.6 candidate commit.
