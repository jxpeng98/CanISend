...
**Status:** Active — authoritative; Alpha.10 remains the Codex-qualified public checkpoint,
Beta.1 source is staged at `beta-qualifying`, and exact candidate construction is next
...
at `cd40180f2ff8ac957276f1948ba88da428511a82`, published 2026-08-29. Its same-byte promotion and
public assets are verified, and `canisend.provider-dogfood/v2` binds the two required Codex Pack
scenarios. Alpha.9 remains an immutable historical checkpoint under the earlier three-host policy.

**Current machine stage:** Beta / `beta-qualifying`; the Beta.1 candidate, publication,
qualification, and feature-freeze activation remain pending; no RC is recorded and Stable is not
authorized.
...
**Next intended checkpoint:** `v1.0.0-beta.1` may freeze the published Alpha.10 Workspace v4,
Application-level Pack binding, connected intake, App/CLI/MCP surface, and Agent v4/Skills
contract. Lean readiness and `canisend.beta-contract-freeze/v2` now bind that exact checkpoint;
the source-stage transition is applied, while the exact candidate, publication, and qualification
remain pending. The invited cohort runs on public Beta.1 and remains required before RC. Beta is
not yet published or qualified.
...
Lean Beta.1 readiness and its exact v4 contract freeze are now qualified; the separate
source-stage transition is applied and exact candidate construction is next. Invited-user
...
|---|---|---|
| Beta.1 candidate work is pending in Issue #74 | P0 before Beta | A staged Beta source does not build native packages, publish Beta.1, or establish qualification |
...
Codex-first qualification evidence. `M4-READY-001` / Issue #71 and `M4-FREEZE-001` / Issue #72
are Verified through protected PRs #201 and #202; `M4-STAGE-001` / Issue #73 owns the staged
Beta.1 source transition, and `M4-CANDIDATE-001` / Issue #74 is next after its protected merge.
...
13. **Completed 2026-08-29:** applied the reviewed Beta.1 source-stage transaction with matching
    preview/write reports and unchanged Alpha.10 authorities; candidate and publication work were
    not started.
14. **Next:** build and publish one build-once Beta.1 candidate, independently reverify it, record
    qualification, activate feature freeze, then complete `M3-EVID-005` / Issue #70 before RC.1.
15. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
