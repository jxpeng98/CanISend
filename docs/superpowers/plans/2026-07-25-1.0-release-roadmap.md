...
separate. Accepted ADRs remain authoritative for product and architecture decisions.
...
| Codex, Claude Code, and Claude Desktop | Supported external hosts; Code hosts use new v4 Skills and Desktop uses the same local MCP | Codex CLI is the required release host for both Packs; generated Claude compatibility remains source-checked and its real-host sessions are non-blocking observations |
...
20. **Next:** complete `M3-EVID-005` / Issue #70 before RC.1 planning.
21. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
