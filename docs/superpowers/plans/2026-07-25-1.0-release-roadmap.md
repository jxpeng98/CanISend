...
**Status:** Active — authoritative; Alpha.10 is the Codex-qualified public checkpoint, lean Beta.1
readiness and its v4 contract freeze are qualified, and the Beta.1 stage transition is next
...
the stage transition and Beta candidate remain pending. The invited cohort runs
on Beta.1 and remains required before RC. Beta is not yet published or qualified.
...
stage/candidate path is next. Invited-user evidence runs on public Beta.1 before RC.1. None of
these tasks may create a parallel engine, a second approval system, or rewrite historical Alpha
evidence.
...
|---|---|---|
| Beta.1 stage/candidate work is pending in Issues #73–#74 | P0 before Beta | Qualified readiness and contract freeze do not change stage, build native packages, or publish Beta.1 |
...
Codex-first qualification evidence. `M4-READY-001` / Issue #71 has a qualified v2 readiness record;
`M4-FREEZE-001` / Issue #72 has the exact v2 freeze record pending protected merge;
`M4-STAGE-001` / Issue #73 is next. `M3-EVID-005` / Issue #70 runs after
public Beta.1 and before RC.1. When sources disagree,
...
13. **Next:** apply the reviewed Beta.1 stage transition, publish one build-once candidate,
    independently reverify it, record qualification, and activate feature freeze.
14. Run the invited cohort on public Beta.1 and complete `M3-EVID-005` / Issue #70 before RC.1.
