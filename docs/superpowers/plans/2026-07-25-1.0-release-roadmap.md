...
| Post-Alpha.6 `main` | Contains the clean Workspace v4/Agent v4 implementation, mixed-Pack Applications, connected intake, App bootstrap, guarded lifecycle, and retired public legacy surfaces; exact Alpha.7 qualification and publication remain pending |

The v4 application facade is now shared by the desktop, CLI, and MCP surfaces. Remaining Alpha.7
work is convergence: remove stale active release truth, close only evidenced end-to-end gaps,
qualify exact packaged bytes, and validate real hosts. It must not create a parallel engine or a
second approval system merely to make every adapter expose identical commands.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Why it blocks progress |
|---|---|---|
| Active version, support, release-note, and package facts still include compatibility-era Workspace/Agent identities | P0 before Alpha.7 | Exact Alpha.7 bytes must report Workspace v4 and Agent v4 without rewriting Alpha.6 history |
| The full desktop mixed-Pack lifecycle is not yet retained as exact packaged interaction evidence | P0 before Alpha.7 | Source tests and local design previews do not alone qualify the public App journey |
| The new packaged Agent v4 lifecycle harness still needs protected Linux, Windows, and macOS CI | P0 before Alpha.7 | Local Apple Silicon preflight does not establish cross-platform packaged CLI ownership |
...
1. Merge and inspect the protected CI result for the packaged Agent v4 host/MCP lifecycle slice.
2. Run one M3 convergence audit against the current operation registry and Issues. Close evidenced
...
3. Cut active development truth from Workspace/Agent v2/v3 to v4 in one vertical PR: version
   output, support policy, release notes, package contract, docs, and their existing source checks.
   Preserve Alpha.6 tag-scoped history unchanged.
4. Complete one exact packaged desktop journey covering bootstrap, both Packs, connected intake,
   explicit associations, Requirement/Plan/Deliverable approval, reopen, recovery, and export.
   Fix only failures found by that journey; do not add new workflow features.
5. Freeze the Alpha.7 candidate source. Run Codex and Claude Code dogfood against the exact draft
   candidate, using CLI for initialization/recovery and persistent MCP for guarded work.
6. Build once, qualify the five CLI targets and supported macOS App packages, promote the same
   bytes, download them, and publicly reverify `v1.0.0-alpha.7` (`M3-ALPHA7-001`).
7. Run the invited Alpha.7 cohort and body-free evidence. Only P0/P1 observed blockers may change
   the candidate line; defer feature requests.
8. Refresh Beta readiness from qualified Alpha.7, publish Beta.1, and activate feature freeze.
9. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
