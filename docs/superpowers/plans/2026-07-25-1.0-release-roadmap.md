...
|---|---|---|
| Post-publication bootstrap usability fixes are green in Draft PR #174 but are not published Alpha.7 bytes | P0 before cohort entry | The maintainer must disposition the observed blockers and bind the cohort to public Alpha.7 or a newly qualified replacement checkpoint |
...
qualification/evidence child beside it. The current delivery task is `M3-EVID-005` / Issue #70;
create `M4-READY-001` / Issue #71 only after the M3 exit gate passes. When sources disagree,
correct the lower-authority projection before advancing the affected gate.
