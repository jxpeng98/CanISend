...
| LF-C05 | LF-C04 | One Host completes source intake, Application preparation, candidate review, export/recovery; another local session resumes canonical state | Native False/True evidence retained; automated dual-Pack flow and exact reopen/restore pass. Full real Host journey and resumption remain pending |
...
| LF-C08 | LF-C06 | Competing claims/commits, stale inputs, duplicate operations, cancellation, crash and recovery refuse invalid writes | Planned |
