...
`M3-HEADLESS-001` / Issue #193 is the current critical-path product child. The expired dependency
gate is isolated as `M3-DEPS-001` / Issue #195 and must pass before the headless source gate can
close. `M3-ALPHA10-001` / Issue #194 follows only after the protected headless merge.
