...
| CLI build and delivery independence | LF-C01–03 | Existing CLI dependency tree has no GUI dependency on the audited host; default workspace selection and delivery preflight still include desktop work |
...
|---|---|---|---|
| LF-C01 | Direction decision | Reconcile baseline, previous work and real CLI dependency/function boundaries; select a bounded next PR | In progress |
| LF-C02 | LF-C01 | Explicit CLI default build/test selection without frontend prerequisites; preserve command behavior | Planned |
...
The next implementation is LF-C02 after integration and LF-C01 review. LF-C10 does not wait for
