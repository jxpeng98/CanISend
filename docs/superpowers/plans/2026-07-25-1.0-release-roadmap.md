...
`M3-ALPHA9-001` is the next checkpoint task but remains Planned until its entry and release actions
are explicitly authorized. `M3-EVID-005` / Issue #70 waits for real invited users and the exact
qualified Alpha.9. Create `M4-READY-001` / Issue #71 only after the M3 exit gate passes. When
sources disagree, correct the lower-authority projection before advancing the affected gate;
Alpha.8 history remains immutable.
...
9. Review and explicitly authorize `M3-ALPHA9-001`, then qualify the next sequential Alpha from
   the exact protected source before affected cohort evidence resumes.
10. Run the invited cohort and body-free evidence on the latest exact qualified Alpha. Only P0/P1
