...
The active development source now reports `1.0.0-alpha.7`; changed bytes still require an exact
candidate before they may be described as a published Alpha.7.
...
|---|---|---|
| Canonical v4 review/export and the final uncovered adapter bindings remain incomplete | P0 before Alpha.7 | Declared operation IDs must not imply cross-surface runtime support before their guarded implementation and exact qualification |
...
| Exact Alpha.7 host-resource tamper/version/legacy fixtures and five-target packages remain unqualified | P0 before Alpha.7 | Protected development CI proves three OS families, not the clean-tag native candidate matrix |
