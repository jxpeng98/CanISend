...
Application-scoped candidate and link inventories through the standalone CLI and read-only MCP
server. They require an exact Application ID and do not imply consent or create a link. Guarded
association preview/commit remains intentionally unavailable from CLI/MCP until the Agent v4
single-use approval-token boundary is implemented end to end.
