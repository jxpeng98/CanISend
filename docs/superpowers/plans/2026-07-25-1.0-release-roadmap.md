...
**Next intended checkpoint:** `v1.0.0-beta.5` is the active source version for the owner-directed
single-package npm testing prerelease. Publish the locally verified CLI through npm's `next` channel;
declare only the native platforms actually supplied. GitHub CI publication is deferred to the owner.
