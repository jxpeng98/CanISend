...
Before Beta.1, against exact qualified Alpha.10, complete `M4-READY-001` and use the lean
readiness writer reviewed in that task. The current cohort-bound refresh script must not be used
to claim Beta entry until that contract is replaced. Then run:
