...
**Status:** Active — authoritative; Beta.1 is the current build-once, publicly qualified
checkpoint at `beta-qualifying`, feature freeze is active, the private Beta.2 native candidate is
independently verified, and invited-user evidence is the current pre-RC checkpoint
...
**Reconciled after Alpha.10 same-byte publication:** 2026-08-29

**Reconciled after Beta.1 source-stage preparation:** 2026-08-29

**Reconciled after Beta.1 qualification:** 2026-08-30

**Reconciled after Beta.1 package-channel candidates:** 2026-08-30

**Reconciled after Beta.1 feature-freeze activation:** 2026-08-30

**Reconciled after private Beta.2 source preparation:** 2026-09-03

**Reconciled after private Beta.2 native candidate verification:** 2026-09-04

**Reconciled for the App-first Agent client direction:** 2026-09-04

**Reconciled after R0/R1 and the execution-process review:** 2026-09-04

**Reprioritized to CLI-first local Agent workflows:** 2026-09-06
...
defines the post-Alpha.6 Workspace v4, Agent v4, and no-legacy-compatibility direction.
[ADR-RN-0021](../../architecture/rust-native/decisions/0021-allow-sequential-beta-iterations.md)
permits exact sequential Beta source checkpoints while keeping qualification and publication
separate.
[ADR-RN-0022](../../architecture/rust-native/decisions/0022-make-the-desktop-an-app-first-agent-client.md)
records the historical App-first decision.
[ADR-RN-0023](../../architecture/rust-native/decisions/0023-prioritize-cli-first-local-agent-workflows.md)
supersedes its delivery priority with native CLI-first and same-device Agent workflows. Accepted
...
`4876c5669b7ae48ca053b5e06e0005419d2051f6`. Alpha.10 was published from
`cd40180f2ff8ac957276f1948ba88da428511a82`; candidate and public bytes agree, and Codex CLI passed
the required Academic and Generic Pack scenarios. Claude real-host sessions remain non-blocking
compatibility observations.

**Current public checkpoint:** [`v1.0.0-beta.1`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-beta.1)
at `6e1397b79031cad54e794ccdc9edca2153f23b3e`, published 2026-08-30. Its build-once candidate,
same-byte promotion, 20 public assets, checksums, native signing records, and attestations are
independently verified, and its exact tag/source/run/manifest are recorded as qualified.

**Current machine stage:** Beta / `beta-qualifying`; Beta.1 is public, independently verified, and
qualified, with deterministic local package-channel candidates derived from its exact public
archives. Feature freeze is active at protected repository baseline
`acf25dc483643ca9be0210320775708da116b715` with zero initial exceptions; invited-user evidence
remains pending. No external package index is modified, no RC is recorded, and Stable is not
authorized.
...
**Next intended checkpoint:** `v1.0.0-beta.10` is the active source version for the owner-directed
npm, Cargo and PyPI testing prerelease with clearer CLI commands, consistent shortcuts
and readable output plus Linux GNU x86_64/arm64 registry packages. Beta.9 was published through
the three registry-only paths in `release.yml` run `35473707357`, from
`7581d2f72e4d753e1c220d68e05d5e55ea310fa0`, for macOS ARM64 only.
The [release record](../../../RELEASE.md) retains its exact checksum and verification.
This bounded registry distribution does not claim complete native or real-Host qualification.
The private Beta.2 nonpublishing candidate remains independently verified at protected
source `2ae2b507b953eef3101aa9689bd60f91a0046605`. The latest qualified public checkpoint remains
[`v1.0.0-beta.1`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-beta.1), whose artifact
source is `6e1397b79031cad54e794ccdc9edca2153f23b3e`; the separate protected repository baseline is
`acf25dc483643ca9be0210320775708da116b715`. Historical Beta.2 evidence and the qualification ledger
remain unchanged. The consented body-free cohort is the next required checkpoint before RC.1
planning.

Any future CLI-first validation-build change requires a tested evidence-contract transition under
LF-C10; the current Beta.1 validator is unchanged by this document.
...
Deliverables, collaborates through a qualified external Agent using the native CLI/MCP, explicit
consent and versioned contracts, reviews and renders outputs, exports a package, and can back up,
restore, repair, or audit the Workspace.
...
| Codex and external Agent hosts | Codex remains the required external release host; native CLI and v4 Skills/MCP are the primary path | Embedded expansion is deferred; generated Claude compatibility and real-host observations keep their existing evidence status |
...
| npm/PyPI testing prerelease | macOS arm64 and Linux GNU x86_64/arm64 | Native build, install and registry readback; not full checkpoint evidence |
...
The primary journey requires no CanISend window: install and configure the native package, select
an Application, inspect evidence and requirements, propose and approve exact candidates, then
export or recover through existing operations. Prove one Host and same-device resumption before
adding task claims and candidate review across two actual local sessions. Existing GUI support
remains a maintenance commitment; App-first expansion is deferred, not silently removed.
...
- qualified Linux ARM GitHub Release archives or desktop packages;
- arbitrary external Typst packages or user fonts;
- App-owned provider accounts, credentials, billing, durable transcript bodies, plugins, or search
  indexes;
- multiple simultaneous embedded Agent sessions or a general-purpose Agent/IDE client; and
...
- Embedded Agent sessions use Codex App Server for conversation lifecycle and the existing MCP
  server for CanISend operations; host permission never implies product approval.
- Read-only annotations do not imply non-private content. Private reads/provider transmission,
  product commits, and exports retain separate explicit authority; model booleans are not consent.
- Effective provider isolation is proven before private tools are enabled, including inherited
  configuration and start/resume/turn behavior; a working directory alone is not a boundary.
- Committed body-free origins belong to product audit and survive session-cache deletion. Historical
  file identity uses logical output paths, generator/build provenance, and exact retained bytes.
- Git owns source and release provenance, not private Workspace content. User-visible file history
  covers only exact outputs registered by the managed projection/export pipeline.
...
| Alpha.9 | Published the Store→IO architecture checkpoint and qualified the earlier three-host provider policy from exact source `4876c566` |
| Alpha.10 | Published the headless capability checkpoint from exact source `cd40180f`; same-byte public evidence and both required Codex Pack scenarios pass |

The v4 application facade is shared by the desktop, CLI, and MCP surfaces. Exact Alpha.10 bytes
have been publicly reverified, and Codex CLI passed the required Academic and Generic scenarios.
Lean Beta.1 readiness, exact v4 contract freeze, source-stage transition, build-once public
candidate, qualification ledger, local package-channel candidates, and active freeze are complete.
Private Beta.2 policy/readiness, source preparation, and one independently inspected
nonpublishing native candidate are also complete. Invited-user evidence remains required before
RC.1. None of these tasks may create a parallel engine, a second approval system, modify an
external package index, or rewrite historical Alpha evidence.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Current evidence and next boundary |
|---|---|---|
| Previous-stage integration | Complete | PR #226 merged the reviewed work and ancestral PR #225 with protected CI; R0/R1 retained and partial R2 unaccepted |
| CLI build and delivery independence | LF-C01–03 | CLI is the explicit default; five target dependency graphs have no GUI dependency. Relocated-binary, resource repair and same-build lifecycle checks are implemented; standalone release qualification remains LF-C10 work |
| Trusted headless approval | LF-C04 | Broker is process-local; verify actual confirmation/commit ownership and preserve private-read, commit, and export separation |
| Complete single-Host journey and resumption | LF-C05 | Reuse v4 CLI/MCP and dual-Pack tests; new exact Host/end-to-end acceptance remains pending |
| Same-device Agent collaboration | LF-C06–08 | Claims, candidate/review binding and competing/crashed commit recovery require bounded design and real-session evidence |
| Exact standalone baseline and user evidence | LF-C10 / M3-EVID-005 | Qualify changed bytes independently; explicitly test any evidence-binding transition while preserving cohort thresholds and historical Beta.1 results |
| Collaboration qualification | LF-C11 | Requires LF-C07, LF-C08 and LF-C10; optional Hosts and cross-device work do not block the first standalone baseline |

R0/R1 source completion remains recorded in the retained
[R0 checklist](../../../.trellis/tasks/archive/2026-09/09-04-embedded-agent-app-r0/implement.md) and
[R1 evidence](../../../.trellis/tasks/archive/2026-09/09-04-embedded-agent-app-r1/research/app-server-evidence.md).
R1 source is `8240fae7da0eb5ecafc2e3dce7bf7609d05f9cf3`; the closeout started at
`d7c0abfea46aa8d7b9f63d9b1ac7bd7fac93abea` with existing uncommitted R2 and process changes.
The owner confirmed integration of the latest work branch and main on 2026-09-06. This does not
qualify R2 or authorize publication. The existing [closeout checklist](../../../.trellis/tasks/09-04-embedded-agent-app-roadmap/implement.md#cli-first-closeout-and-lf-c01)
records review, checks, integration, and the next slice. LF-C IDs are mapped here; the downloaded
proposal tasks and static validation are neither execution instructions nor product-test evidence.
Public Issue projection is pending; no synchronization is claimed.

### 3.3 Approved CLI-first delivery sequence

The 2026-09-15 owner request resumes execution toward 1.0 readiness. The next bounded slice is
first-use permission guidance over the existing Host setup and native session grant, followed by
actual Host acceptance and LF-C10 qualification. The
[existing execution plan](../../../.trellis/tasks/09-04-embedded-agent-app-roadmap/implement.md#first-use-permission-guidance-and-10-readiness--2026-09-15)
records its checks and remaining gates. No blanket permission, durable consent, new approval store,
support-matrix reduction or Stable publication authorization is implied.

| Unit | Dependency | Scope and acceptance | State |
|---|---|---|---|
| LF-C01 | Direction decision | Reconcile baseline, previous work and real CLI dependency/function boundaries; select a bounded next PR | Complete — PR #226, merge `03897b71` |
| LF-C02 | LF-C01 | Explicit CLI default build/test selection without frontend prerequisites; preserve command behavior | Complete — PR #227, merge `ce12ad87` |
| LF-C03 | LF-C02 | Verify standalone resources, install/upgrade/repair/uninstall without an App, source checkout or consumer language runtime | Source checks implemented; exact native archive and upgrade-pair qualification remain LF-C10 |
| LF-C04 | LF-C03 | Trusted windowless preview/confirmation/commit in the correct Broker; reject wrong scope, stale/replayed grants and restart reuse | Source checks complete — PR #229, merge `83c9120d`; actual Host confirmation qualification remains LF-C05 |
| LF-C05 | LF-C04 | One Host completes source intake, Application preparation, candidate review, export/recovery; another local session resumes canonical state | Source checks complete: automated dual-Pack flow and exact reopen/restore pass; native False/True evidence retained. Full real Host journey and resumption remain pending qualification |
| LF-C06 | LF-C04 + LF-C05 automated checks | Reuse existing Task/Review/Store for minimal atomic claims and bounded candidate handoff; separate coordination and content revisions | Local source implemented: CLI task discovery, fenced leases and private candidate handoff reuse tasks/Blobs without migration; owning and CLI recovery checks pass. Review/commit integration is recorded under LF-C07 |
| LF-C07 | LF-C06 | Two actual local Host sessions perform distinct tasks and exact-candidate review; reliable user-approved final write and receipt | Local source integrated: persisted candidates enter the existing native draft preview/commit; task completion and Application drafts commit atomically. Synthetic protocol and owning rollback checks pass; actual two-Host qualification remains open |
| LF-C08 | LF-C06 | Competing claims/commits, stale inputs, duplicate operations, cancellation, crash and recovery refuse invalid writes | Source checks complete: synchronized independent-connection races serialize claims/submissions/commits/cancellation; killed reviewer recovery preserves candidates and resolves lost receipts from canonical state |
| LF-C09 | LF-C04 + LF-C05 automated checks | Optional additional Host adapters with individual capability and dual-Pack evidence | Optional |
| LF-C10 | LF-C05 | Qualify exact standalone CLI baseline, clean-machine lifecycle and business flow; tested cohort-binding transition if required | Local Apple Silicon release candidate built and extracted-archive automation passes, including target/identity, isolated consumer, dual-Pack/local handoff, and local Beta.1→Beta.2 state preservation. Actual Host, prescribed native matrix and release qualification remain open; no publication |
| LF-C11 | LF-C07, LF-C08, LF-C10 | Qualify same-device collaboration, real-session evidence, upgrade and recovery | Local extracted-candidate handoff and exact committed-task/private-candidate reopen/restore pass. Actual two-Host sessions and native/release qualification remain open |
| LF-C12 | LF-C11 | Separately decide cross-device migration/synchronization | Deferred; never a local-work prerequisite |

The owner confirmed the first integration stage as **LF-C01–08 source integration and automated
validation** on 2026-09-07. That local stage is complete: standalone CLI/resource checks, both
Pack lifecycles, candidate handoff, concurrent mutation and interrupted-reviewer recovery pass.
The integration evidence is recorded in the existing implementation plan. This does not claim
protected CI, actual Host acceptance, packaged native qualification or release publication;
LF-C05/07 human qualification and LF-C10/11 remain separately open. LF-C09 is optional and LF-C12
remains deferred.

The owner subsequently confirmed that the next integration stage also ends at **local automated
acceptance**. LF-C10/11 local candidate and collaboration recovery automation is complete; their
human/native release qualification remains open without blocking this local milestone. The
simulated MCP Host runs in Fast CI and can target exact extracted candidate bytes; its responses
never count as real user or provider-UI acceptance.

The next owner-authorized development stage is **iterative changes to an existing Application**:
I1 shared update-impact preview, I2 guarded Requirement revision, I3 Source/replan/material recovery,
and I4 dual-Pack revision acceptance. The [existing execution plan](../../../.trellis/tasks/09-04-embedded-agent-app-roadmap/implement.md#iterative-application-changes-execution-plan--2026-09-07)
owns slice scope and evidence. Reuse current invalidation/history rather than resetting Applications;
this development stage does not reopen completed confirmation tests or waive Beta qualification.
I1, the bounded I2 CLI/MCP correction path and I3a re-confirm/replan/material recovery have passed
local checks. Bounded I3b now adds MCP revision of an exclusively linked pasted-text Source,
atomic association/Requirement rebinding and preserved history, with automated denial, rollback,
shared-link conflict and dual-Pack restart checks. Shared/file/URL updates remain outside that
slice. I4 still owns the complete dual-Pack change-to-export journey; it is not yet qualified.

Keep Rust, `canisend-app`, ApprovalBroker, Store/Blobs, both Packs and current v4 identities.
First reuse a single valid Broker for final preview, reliable human confirmation and commit;
workers exchange persisted candidates/results, not active approval tokens. Introduce a local
coordinator only for an evidenced cross-connection need. Do not create a generic daemon or another
workflow database. A terminal, model role or unrestricted same-user shell is not human authority.

LF-C01–04 source slices merged through PRs #226–229. LF-C03 verifies isolated resource and
lifecycle behavior; LF-C04 adds separate Host confirmation for guarded operations. LF-C05 is
an open qualification slice. Follow [automation-first acceptance](../../guides/agent-acceptance.md).
The developing MCP request-form contract, complete Pack catalog read, and source-bound Evidence
confirmation path now pass focused local checks and the dual-Pack smoke. Updated Skill digests
require renewed actual Host evidence at candidate qualification; the full release check reports
the old provider dogfood binding as stale. This does not block development source checks or LF-C06.
The remaining actual Host journey and same-device resumption evidence are pending.
LF-C10 owns exact native qualification and does not wait for collaboration
or extra Hosts. No later LF-C task is implemented by accepting this roadmap.

The 2026-09-07 owner-approved process amendment separates three activities: implementation with
focused automated checks, one source CI pass per integration identity, and candidate qualification.
LF-C01–12 remain traceable work IDs, not twelve manual approval stops. Packaged-binary, actual Host,
cohort and freeze evidence gates the release claim; it is collected at that boundary. Ordinary
development does not repeatedly renew that evidence or ask permission to move to an authorized
next step. Real product consent is still required when operating a user's Workspace.

<a id="33-approved-app-first-delivery-sequence"></a>

### 3.4 Historical App-first sequence — superseded 2026-09-06

The following records the earlier R0-R6 plan and its original acceptance criteria. It is retained
for traceability, not an active work queue. R0/R1 source is complete; R2 is partial and unaccepted;
unfinished R2-R6 scope and M4-APP-001–005 are deferred by ADR-RN-0023. Private embedded tools
remain disabled. Use section 3.3 and LF-C01–12 for current ordering.

Source progress (2026-09-05): R2a has Application-bound MCP and a real Codex 0.152.0 local probe
covering named read restrictions, required-server failure, inherited MCP overrides, and explicit
read acceptance/denial across start/resume/turn. The desktop now applies named permissions before
initialization and on every request, resolves executable aliases, and rejects unqualified versions.
Account recognition and Application session binding pass locally, including registry v1/v2 migration
and stale UI event rejection. Complete inherited-configuration isolation, product consent correlation/UI,
and the signed-in cross-Pack flow remain open. The provider reloads
MCP configuration after inventory, so snapshot-and-disable cannot close that gate. The local probe
also reproduces standalone Skill metadata reaching the model despite the discovery skip flag and
excluded directory read grant. Host-denial notices now appear in the Agent view. The owner approved
separate Codex configuration/login on 2026-09-05; the dedicated child directories and browser-login
button are implemented. A local fixture proves external Skill and MCP configuration exclusion with
that recipe. Real login and managed/system configuration qualification remain pending; embedded
private tools stay disabled.
See [R2a evidence](../../../.trellis/tasks/09-04-embedded-agent-app-r2/research/r2a-boundary-evidence.md).
This is partial source implementation, not R2 acceptance or an artifact qualification.

ADR-RN-0022 retains the App-first direction. The owner requested this optimization on 2026-09-04.
The next product slice is R2a; its detailed checklist already exists in the
[R2 plan](../../../.trellis/tasks/09-04-embedded-agent-app-r2/implement.md). No replacement planning
tree is needed. Product implementation, protected integration, native qualification, and user
validation are distinct completion states. Active-freeze exceptions still bind exact commits;
this plan authorizes neither future commit identities nor a tag, release, or support claim.

```text
R0 authority + clean-v4 fix [source complete]
  -> R1 Codex App Server transport [source complete]
     -> R2a effective isolation + bound MCP + explicit consent/approval
        -> R2b durable audit origin + exact managed-file history
           -> R3 complete Workbench + bounded provider-history display
              -> R4 exact candidate + recovery/accessibility + changed-journey user evidence
                 |-- embedded MVP qualified --|
                 +-> R5 generic ACP + first Claude Agent qualification
                 +-> R6 old-route/runtime cleanup after parity and rollback window
```

| Unit | Owner | Entry | Exit and rollback |
|---|---|---|---|
| R2a / M4-APP-001 | Desktop/application maintainer | R1 source; exact provider protocol proof first | All tool paths and private reads are bound and consented; one verified mutation and negative cases pass. Disable private embedded tools on failure |
| R2b / M4-APP-002 | Store/application maintainer | Stable R2 receipt/binding contracts | Durable origins and exact logical-file snapshots survive cache loss, generator change, repair, and restore. Preserve additive schema/data and use compatible backups for binary rollback |
| R3 / M4-APP-003 | Desktop/product maintainer | R2a and R2b accepted | Both Packs complete context -> evidence -> Deliverable -> review -> exact preview/export; history reload and provider-unavailable manual work are usable. Restore old routes if parity fails |
| R4 / M4-APP-004 | Release/validation maintainer | R3 source; protected CI before packaging | Exact embedded Beta candidate, provider/version capability, recovery and accessibility evidence pass. Disable embedded entry point if qualification fails |
| Embedded user gate / M4-APP-005 | Product/validation maintainer | Define schema/scenarios during R2/R3; collect only after exact embedded Beta qualification and participant consent | Existing Issue #70 thresholds bind the new qualified build through a tested validator transition; missing/stale evidence blocks RC. Keep Beta.1 history immutable |

R2a proves isolation before initialization, start/resume/turn consistency, required MCP readiness,
and safely correlated user approval on the exact supported Codex version. It guards every
Application-ID handler and constrains no-ID/Workspace-wide listing output. Metadata, private
read/provider send, product commit, and export have distinct policies; `writes` approval mode and
model-supplied consent flags cannot authorize private content. Unsupported policy capabilities keep
private tools disabled, while independent R2b preparation may continue.

R2b reuses Store/Blob/audit authority. A committed origin survives registry eviction/deletion and
backup/restore; attachment failure reports a trace gap without repeating a mutation. File snapshots
use logical paths relative to the generation root, actual generator/build identity, and same-kind
predecessors. Only an identical current head with full binding is reused; A -> B -> A retains its
new history link. Repair/restore reads exact retained Blobs. Bounded manifest-first/selected-file
comparison may lock the previously qualified `similar` 3.2.0 only with its implementation and actual
lockfile checks. No Git, Workspace scan, transcript database, or persisted patch is added.

R3 enriches the workflow through existing product capabilities rather than adding more engines:

- Work shows pack stages, readiness blockers, next actions, and selectable Deliverables with draft,
  review, validation, and export state.
- The Inspector joins Requirement-to-Evidence coverage, missing/stale associations, explicit context
  selection/consent, proposal review, durable origin, exact file comparison, and PDF preview.
- Guided analyze/draft/revise/check/export actions use the same facade/Skills contracts as free-form
  conversation. Library and Settings remain the other primary destinations.
- Reopen/resume loads bounded history for the known provider thread into memory with stable ordering
  and deduplication. Provider-owned retention or unavailable history is explicit; no local transcript
  store is created. Supported manual inspection/review/export still works without the provider.

R4 connects this sequence to the M4 exit gate and the exact build under review. A recorded macOS
smoke cannot imply other native targets; earlier PDF qualification remains regression evidence,
not qualification of new App bytes. Keep Tauri and exact-byte preview; Electron/PDF.js remain behind
the measured ADR gate. R5 starts after R4; a provider trait is extracted only with the second real
implementation. R6 depends on parity and the rollback window, not on completing R5.
...
An existing plan or a bounded local Roadmap row may supply this metadata; a separate task system is
not required. Public Issue projection may remain explicitly pending during authorized local work.
It must not be reported as completed GitHub governance. Each work item carries:
...
| Issue | Existing URL/number, or explicitly pending public projection |
...
- Keep one integration slice active. Independent preparation or qualification may overlap when
  contracts and file ownership are stable; competing schema/approval changes remain sequential.
- R2a and R2b share one R2 plan. R3 may prepare presentation against accepted read contracts, but
  cannot enable private tools or claim the end-to-end journey before R2 passes.
...
- Alpha.6 is a dual-Pack framework and migration checkpoint; it publishes the already implemented
  Generic Pack and canonical v3 surfaces, but it cannot authorize Beta or substitute for
  post-Beta user validation.
...
- Alpha.7 candidate work starts only after clean-v4 App, CLI, MCP, Codex, Claude Code, and Claude
  Desktop flows pass.
- Mixed-Application invited-user evidence is currently bound to public Beta.1. LF-C10 must
  explicitly update the existing validator before the formal cohort targets a qualified CLI-first
  Beta. Keep one formal cohort gate with unchanged thresholds; synthetic dogfood contributes zero users.
...
|---|---|---|---|
| D — prose/history | `git diff --check`; active release documents also run the relevant `xtask` check | Fast CI when synchronized remotely | Rust tests, Clippy, package builds |
...
| V — vertical product slice | One kernel/facade invariant test plus one end-to-end adapter smoke; source gate once at a meaningful integration milestone | Fast CI when synchronized remotely | Repeating the same business assertion in every adapter |
...
contract, facade, adapter, UI, documentation, and primary tests in local commits. Reuse the current
branch; create branches/worktrees only for conflicting parallel work or risky experiments. Merge
completed branches locally, preferably by fast-forward. Before coding, reuse the existing facade, approval broker, semantic matrix,
...
3. **Integrate locally:** at a completed feature, contract change or substantial merge, update the
   existing record and run the owning checks, including `xtask source check` when applicable.
   Do not rerun suites per edit/commit or after an unchanged clean fast-forward. No PR or human
   Host session is required to continue local development.
4. **Qualify:** synchronize the coherent milestone when needed, inspect the required remote CI,
   then run the complete `xtask release check` for a
   candidate. This step is independent of starting the next development slice. Run native
   packages only when package/runtime bytes changed or the exact Alpha/Beta/RC/Stable candidate
   is frozen.

Create a PR only for requested external review or when protected remote integration requires it;
group related local work into that review. Local branch, commit and merge operations require no
extra process approval within the authorized scope. Do not alter remote protections or force-push
to avoid a required review. Feature-branch pushes do not run a second copy of Fast CI. Failed
checks are rerun after a relevant change; unchanged successful jobs are not recreated locally.

### 5.7 Direct project control

The owner removed project-local Trellis skills, agent adapters, and hooks on 2026-09-04. Follow
[AGENTS.md](../../../AGENTS.md) and the retained
[project control guide](../../../.trellis/spec/guides/project-control.md). The master roadmap still
owns ordering; ADRs and machine records still own their facts. Historical plans/specs/evidence keep
their paths and remain readable without Trellis.

Use one existing execution checklist per outcome with owner, scope, acceptance, dependencies,
rollback, actual evidence, and next action. A new task directory, phase activation, agent dispatch,
journal, or separate bookkeeping commit is not required. Update shared contracts and the checklist
once with the completed slice; do not reproduce the same plan or test matrix in multiple systems.
Continue already authorized work without repeated process approvals. Ask only for a material missing
decision or an action outside existing authorization.

Task status fields are descriptive metadata. They never substitute for protected CI, exact package
qualification, consented user evidence, or release authorization. Do not regenerate deleted Trellis
adapters unless the owner explicitly reinstalls that workflow. Product Agent v4 Skills and
user-global tools are outside this removal.

The complete candidate release check retains feature-freeze validation. Collect exact post-baseline
dispositions against candidate history at qualification; they are not per-edit approval stops.
Do not fabricate commits or alter a qualification ledger to hide missing evidence.

`M3-DEPS-001` / Issue #195 and `M3-HEADLESS-001` / Issue #193 are Verified through protected
PR #196 and archived. `M3-ALPHA10-001` / Issue #194 is Verified by the exact public release and
Codex-first qualification evidence. `M4-READY-001` / Issue #71, `M4-FREEZE-001` / Issue #72,
and `M4-STAGE-001` / Issue #73 are Verified through protected PRs #201, #202, and #203.
`M4-CANDIDATE-001` / Issue #74 is Verified by exact public Beta.1 and protected PR #204.
`M4-LEDGER-001` / Issue #75 is Verified by a fresh public download and matching dry-run/write
ledger transaction. `M4-CHANNEL-001` / Issue #76 is Verified by exact regenerated package
metadata and protected reconciliation. `M4-FREEZE-002` is Verified at the exact protected
repository baseline while retaining the qualified Beta artifact source. `M4-BETA2-001` and
`M4-BETA2-002` are Verified through protected PRs #219 and #220. `M4-BETA2-003` is Verified by
private candidate run `33824463477`, artifact `9920609356`, and independent inspection against
protected source `2ae2b507b953eef3101aa9689bd60f91a0046605`. `M3-EVID-005` / Issue #70 is the
current pre-RC evidence task. When sources disagree, correct the lower-authority projection before
advancing the affected gate; historical Alpha evidence remains immutable.
...
flowchart LR
    ...
    M3 --> M4["M4: Beta.1, freeze, and private Beta.2"]
    M4 --> CLI["LF-C01–05: independent CLI and one Host"]
    CLI --> QUAL["LF-C10: exact standalone qualification"]
    CLI --> LOCAL["LF-C06–08: same-device collaboration"]
    LOCAL --> COLLAB["LF-C11: collaboration qualification"]
    QUAL --> COLLAB
    QUAL --> COHORT["Issue 70: exact-build cohort and validated evidence"]
    COHORT --> M5["M5: RC.1 and RC.2"]
    M5 --> M6["M6: 1.0 Stable"]
...
| M4 — Beta baseline, CLI-first delivery, and validation | Re-estimate after LF-C01 audit and trusted approval proof | M3 | Qualified CLI-first Beta, preserved history, freeze disposition, and exact-build user evidence |
...
The earlier 14–24 implementation-week estimate and 2–4 week validation window describe the
earlier scope; they are historical planning context, not an estimate for the CLI-first revision.
Re-estimate from LF-C01, the trusted approval boundary, and the first complete Host journey.
Do not attach an unsupported completion date to the expanded M4. Trusted Developer ID/notarized
distribution adds credential and qualification lead time. Public Windows/Linux GUI support is not part of that
estimate.
...
those bytes as a bounded starter Pack instead of pretending they are absent. Later milestones own
the Beta baseline followed by broad real-user evidence, non-academic scenario coverage, and
feedback-driven hardening before RC.
...
| [M3-DEPS-001](https://github.com/jxpeng98/CanISend/issues/195) | P0 | Unchanged 751-package lock and explicit maintainer review | Re-review the 23 expired lock-bound dependency exceptions without weakening their guarded reachability | Exact `cargo-deny 0.19.5`, dependency policy, and release source gate pass; review and hard expiry are 2026-09-07, and any lock/reachability drift blocks |
| [M3-HEADLESS-001](https://github.com/jxpeng98/CanISend/issues/193) | P0 | Exact public Alpha.9, verified M3-CLI/Agent v4 surfaces, M3-DEPS-001 | Close the App-absent clean-v4 journey using the existing standalone CLI, canonical Skills, application facade, and persistent MCP | CLI exposes project/global host scope; the packaged smoke proves starter resources, both Packs, guarded work, export, backup, restore, reopen, failure cases, and no direct authority writes |

Protected source completion (2026-08-24): PR
[#196](https://github.com/jxpeng98/CanISend/pull/196) merged as
`549072185b5a4886a1b67c1217b79a67d237f826`. Fast CI run
[`32675537169`](https://github.com/jxpeng98/CanISend/actions/runs/32675537169) passed desktop UI,
browser accessibility, Linux, Windows, macOS quality, and macOS tests; dependency-assurance run
[`32675537171`](https://github.com/jxpeng98/CanISend/actions/runs/32675537171) passed the exact
23-entry policy and deny scan on the same PR head. This verifies M3-DEPS-001 and
M3-HEADLESS-001 source integration; it does not qualify or publish Alpha.10.
...
### 11.6 Slice E — Alpha.7–Alpha.10 qualification and user cohorts
...
| [M3-ALPHA10-001](https://github.com/jxpeng98/CanISend/issues/194) | P0 | Protected and verified M3-HEADLESS-001 | Qualify and publish exact `v1.0.0-alpha.10` as the headless capability checkpoint | Protected source, build-once native candidate, exact-host evidence, same-byte promotion, independent public verification, and reconciled authorities agree without rewriting Alpha.9 or claiming invited-user evidence |
...
Alpha.10 qualification evidence: [public release](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-alpha.10),
[candidate run](https://github.com/jxpeng98/CanISend/actions/runs/32678848156),
[same-byte promotion and public verification run](https://github.com/jxpeng98/CanISend/actions/runs/33267148891),
the immutable [publication-gap note](../../notes/rust-native/2026-08-29-alpha10-public-qualification-gap.md),
the [Codex-first qualification note](../../notes/rust-native/2026-08-29-alpha10-codex-first-qualification.md),
and `release/provider-dogfood.json`. Codex CLI passed the required Academic and Generic Pack
preview/cancel scenarios without mutation or submission. Claude Desktop's pass, Claude Code's
`skipped-by-maintainer` result, and the bounded MCP result remain accurate non-blocking
observations. M3-ALPHA10-001 is complete when this reconciliation reaches protected `main`.

| Cohort | Minimum | Purpose |
|---|---:|---|
| Post-Beta invited cohort | 5–8 users and at least 10 complete flows across both Packs | Find initialization, mixed-Application, intake, consent, recovery, and export blockers on the exact qualified validation build |
| Pre-RC cumulative cohort | 8–12 users and at least 20 complete flows | Establish repeatability across two Packs in shared Workspaces, three non-academic families, languages, inputs, hosts, and recovery before RC.1 |

The current cohort contract binds Beta.1. The revised execution target is one formal cohort on the
qualified CLI-first Beta after LF-C10 updates that binding and its machine checks. Preserve old
observations by build; do not require a completed obsolete-UI cohort first or pool its success rate
into the new CLI-first result. The existing sample/coverage/quality thresholds below remain intact.
...
| M3-EVID-004 | P0 | Add a machine-validated Codex-first provider-dogfood record bound to source/artifact, Agent v4, Skills digest, both Pack identities, consent, scenario IDs, and outcomes | Beta preparation rejects a missing Pack scenario, unsafe outcome, stale contract/digest, failed status, or private-field injection |
| M3-EVID-005 | P0 | After Beta.1, record cohort size, mixed-Application/scenario coverage, completed-flow count, exact build, metric numerators/denominators, exclusions, and evidence-note digest | RC entry rejects missing, stale, inconsistent, single-Pack-only, or unbound user evidence |

`M3-EVID-005` / Issue #70 retains its identifier and is the single formal M4 user-evidence gate,
not a Beta-entry gate. Its currently implemented contract binds Beta.1. LF-C10 prospectively
updates that same evidence path for a qualified CLI-first Beta; historical records stay immutable.
Old UI/provider dogfood counts cannot silently satisfy the new surface, and this documentation edit
does not make the current validator accept a different build.
...
- [x] Exact public Alpha.10 binds the protected headless capability, same-byte public artifacts,
  and both required Codex Pack scenarios; Claude real-host sessions are non-blocking observations.
- [x] A clean App setup initializes Workspace, minimum basic data, Packs, MCP, and new host Skills
  without terminal use; the same result can be initialized headlessly by CLI.
- [x] One Workspace contains academic and generic Applications concurrently, and neither workflow
  clears, relabels, leaks, or invalidates the other.
- [x] URL, text, text-PDF, and local-file intake feed explicit Source, Requirement, Evidence, and
  Application associations for both Packs.
- [x] App, CLI, MCP, and required Codex flows pass the v4 semantic boundary; generated Claude
  resources use the same contracts, and App-closed operation/reopen reconciliation passes.
- [x] Old Skills, Agent v2/v3, job aliases, and Workspace v2/v3 are absent from the supported v4
  Alpha matrix and fail closed before mutation when detected.
- [x] Every audited synthetic factual claim is traceable to confirmed Evidence and unsupported claims equal
  zero.
- [x] Mixed-Application backup/restore succeeds in the qualified recovery scenarios.
- [x] No unresolved P0/P1 data-loss, privacy, evidence, Pack, rendering, recovery, host-setup, or
  supported-install blocker remains.
- [x] No blocking keyboard, VoiceOver, or 200%-scale issue remains in the Alpha.10 qualification.

<a id="12-m4--beta-baseline-embedded-app-and-feature-freeze"></a>

## 12. M4 — Beta baseline, CLI-first delivery, and feature freeze

**Objective:** Preserve the qualified Beta.1 baseline and public contract freeze while delivering
and qualifying CLI-first delivery under exact exceptions before RC.

**Entry:** M2 and M3 are Verified. Public Alpha.10 has no unresolved applicable P0 blocker and is
the current Codex-qualified Alpha.

| ID | Priority | Deliverable | Verification |
|---|---|---|---|
| M4-READY-001 | P0 | Replace the pre-Beta cohort dependency with lean readiness against exact Alpha.10 Codex evidence, reviewed maintainer validation, and applicable blocker classes | Snapshot is no older than 24 hours, binds Workspace v4, Agent v4, Skills and both Pack digests, is body-free, reports zero synthetic users, and does not treat zero public Issues as its only evidence |
| M4-FREEZE-001 | P0 | Run `release freeze-candidate` for Agent v4, Workspace v4, Skills manifest/resources, Pack v1, both built-in Packs, schemas, operation registry, exit codes, and bundle layout | Baseline digests match the qualified Alpha.10 surface; no Alpha.6, v2, or v3-only baseline can pass |
...
| M4-FREEZE-002 | P0 | Activate feature freeze at the exact protected post-qualification repository baseline while retaining the qualified Beta artifact source | Ledger contains a non-null repository baseline, unchanged Beta source, and only approved change classes |
| M4-BETA2-001 | P0 | Add exact sequential-Beta governance and prove the existing CLI, four Skills, persistent MCP, and Workspace v4 path in one disposable App-closed journey | Beta.1→Beta.2 is the only accepted next Beta; negative transitions fail closed; source gate and Fast CI pass without live data or global host mutation |
| M4-BETA2-002 | P0 | Preview and prepare the private Beta.2 source through the controlled two-commit freeze-exception protocol | Beta.1 moves unchanged to `beta_history`, active Beta becomes pending, freeze remains active, RC.1 stays deferred, and no tag, release, package, or qualification is created |
| M4-BETA2-003 | P0 | Build and independently inspect one exact nonpublishing Beta.2 native candidate | Every candidate job, target archive, App package, checksum, manifest, SBOM, signing record, and provenance attestation binds one protected source; no tag, release, package publication, or qualification is created |
| M3-EVID-005 | P0 | Complete the one formal exact-build cohort before RC.1; current binding is Beta.1 until LF-C10 prospectively targets the qualified CLI-first Beta | Body-free evidence binds the validator-approved build, both Packs, at least 8 users (target 8–12), at least 20 flows, mixed Workspaces, three non-academic families, metrics, exclusions, and no-submission understanding |
| M4-APP-001 (deferred) | P0 | R2a: effective provider isolation, all-tool Application binding, explicit private-context and mutation authority | Exact-version protocol proof plus owned positive/negative fixtures; missing capabilities keep private tools disabled |
| M4-APP-002 (deferred) | P1 | R2b: durable audit origin and exact managed-file history | Cache loss/restore, logical paths, generator identity, A -> B -> A, Blob-backed repair, export cleanup, and bounded comparison pass |
| M4-APP-003 (deferred) | P1 | R3: complete Workbench and provider-history display using existing domain operations | Both Packs complete guided and conversation-led work; missing/stale Evidence, Deliverable selection, restart/history gaps, and manual provider-unavailable actions are usable |
| M4-APP-004 (deferred) | P0 | R4: qualify exact embedded Beta App/runtime and affected contracts | Protected source, exact candidate, provider capabilities, native recovery/accessibility, and exact preview/export evidence bind the same build; no new support matrix is inferred |
| M4-APP-005 (deferred) | P0 | Update the existing M3-EVID-005 evidence contract/validator for the exact qualified embedded build and changed journeys | Missing/stale/wrong-build and mixed-build denominator rejection pass; unchanged cohort thresholds and immutable Beta.1 history remain, without a second formal cohort gate |

`M4-READY-001` completed its machine record on 2026-08-29: `canisend.beta-readiness/v2`
binds exact public Alpha.10, both required Codex scenarios, host/Pack/Skill digests, body-free
maintainer review, nine clear blocker classes, and zero claimed users or user flows.

`M4-FREEZE-001` completed `canisend.beta-contract-freeze/v2` on 2026-08-29. The record binds that
same Alpha.10 source to Agent/Workspace/resource/Pack/Skill contracts, migrations through 20,
55 checked-in schemas, operation and exit mappings, and the exact Alpha package layouts. It does
not change the machine stage or authorize Beta publication.

`M4-STAGE-001` staged `1.0.0-beta.1` / `beta-qualifying` on 2026-08-29 through one reviewed
28-file transaction. Preview and write reports matched, while qualified Alpha.10 readiness,
contract freeze, feedback, and checked-in candidate history remained byte-identical. This does
not build, publish, qualify, or freeze Beta.1.

`M4-CANDIDATE-001` completed on 2026-08-30. Candidate run `33281162734` built exact protected
source `6e1397b79031cad54e794ccdc9edca2153f23b3e` once; promotion run `33283530240` reused artifact
`9723581536` without recompilation and published `v1.0.0-beta.1`. A fresh independent download
verified all 20 public assets, checksums, manifest, native signing records, and attestations as
byte-identical to the candidate. The
[body-free evidence note](../../notes/rust-native/2026-08-30-beta1-candidate-publication.md)
records that task's public-but-unqualified boundary; qualification followed separately and
feature freeze remains pending.

`M4-LEDGER-001` completed on 2026-08-30 from a new public download. All 20 assets and 20
attestations reverified against the exact release workflow and source. Matching dry-run/write
reports recorded candidate run `33281162734`, manifest SHA-256 `2435c335...`, and ledger change
`7a1599bd...` → `7bff07d6...`; write mode changed only the ledger. The
[body-free qualification note](../../notes/rust-native/2026-08-30-beta1-qualification.md) records
the exact transaction. At that task boundary, package channels and feature-freeze activation
remained pending.

`M4-CHANNEL-001` completed on 2026-08-30 from the same independently downloaded and reverified
public Beta.1 assets. The existing generator wrote one deterministic six-file Homebrew, Scoop, and
WinGet candidate tree whose source record SHA-256 is `d9558872...`. It preserves immutable 0.7
metadata while describing the 1.0 domain-neutral framework, uses GPL-3.0-only, and keeps
`candidate_only: true` / `publication_authorized: false`. The
[body-free channel evidence note](../../notes/rust-native/2026-08-30-beta1-package-channel-candidates.md)
records exact input and output digests. No external package index was modified; feature-freeze
activation remained pending at that task boundary.

`M4-FREEZE-002` activated the existing two-file freeze transaction on 2026-08-30. Protected
preparation PR #207 and state-independent release-policy PR #208 merged before the baseline; the
latest protected merge `acf25dc483643ca9be0210320775708da116b715` is the repository baseline.
Matching dry-run/write reports changed only the qualification ledger and exception record,
retained Beta artifact source
`6e1397b79031cad54e794ccdc9edca2153f23b3e`, preserved the three allowed change classes, and
started with zero exceptions. The
[body-free activation note](../../notes/rust-native/2026-08-30-beta1-feature-freeze.md) records
the exact transaction. This does not create a tag, publish a release or package channel, start an
RC, authorize Stable, or claim invited-user evidence.

On 2026-09-03 the maintainer selected private Beta.2 source preparation and deferred RC.1.
ADR-RN-0021 limits this to an exact qualified Beta.N→Beta.N+1 transition under the active freeze.
Protected PR #219 completed `M4-BETA2-001`; protected PR #220 completed `M4-BETA2-002` and prepared
source `1.0.0-beta.2` while preserving Beta.1 qualification history. PR #221 archived that work.
`M4-BETA2-003` completed on 2026-09-04. Candidate run `33824463477` built protected source
`2ae2b507b953eef3101aa9689bd60f91a0046605` for five CLI targets plus the supported macOS App
packages. Independent inspection accepted all 19 manifest-managed files and all 20 provenance
attestations from artifact `9920609356`. The
[body-free candidate note](../../notes/rust-native/2026-09-04-beta2-private-candidate.md) records
the exact identities and community-signing limits. Beta.2 remains untagged, unpublished, and
unqualified; none of these tasks authorizes promotion or qualification.

### CLI-first validation before RC

LF-C10 reuses the existing build-once qualification and M3-EVID-005 / Issue #70 evidence owners.
Qualify an explicitly authorized exact CLI-first Beta after source review and protected CI. Record
source, artifact/manifest, target, Host capabilities, Workspace, Pack and operation identities.
Any change from the implemented Beta.1 cohort binding requires a tested contract/validator change
before new results count. Cover missing/stale/wrong-build evidence, private fields and mixed-build
denominators. This direction edit changes no release JSON or validator.

Run the formal cohort on the selected qualified build with the unchanged minimum of 8 users and
20 supported flows (target 8–12 users). Preserve historical Beta.1 observations and existing
coverage/quality thresholds, including 80% unassisted completion, 100% audited traceability,
measured recovery and no-submission understanding, and zero unsupported claims or unresolved
P0/P1 blockers. Maintainer/provider dogfood contributes zero users. Reuse consented participants,
but do not pool incompatible builds or double-count a participant. Existing supported GUI scenarios
remain covered until an explicit support/evidence change; embedded expansion is deferred.

Source isolation, clean-machine runtime, trusted headless approval, dual-Pack business completion,
resumption, export and recovery must each have evidence. LF-C11 separately qualifies actual local
collaboration; LF-C12 is outside this release path. Missing required evidence keeps RC pending.

### M4 exit gate

- [x] The ledger contains a qualified Beta tag/source/run.
- [x] Feature-freeze baseline equals the exact protected post-qualification repository `HEAD`, and
  the ledger retains the qualified Beta artifact source unchanged.
- [x] Agent v4, Workspace v4, Skills, Pack v1, both built-in Packs, schema, operation, approval,
  bundle, and exit-code contracts are digest-bound.
- [x] Beta packages and public docs match.
- [x] New feature requests are assigned to 1.1 unless a documented release exception resets the
  affected evidence.
- [x] Exact sequential-Beta policy and the App-closed CLI/Skills/MCP/Workspace gate are merged.
- [x] Private Beta.2 source is prepared with Beta.1 preserved in history and no publication side effect.
- [x] Exact private Beta.2 native candidate bytes pass independent integrity and provenance checks
  without a tag, release, package publication, or qualification write.
- [ ] LF-C02–05 acceptance and LF-C10 exact standalone qualification pass, with trusted approval,
  dual-Pack preparation, installation, resumption, export and recovery evidence.
- [ ] LF-C10 updates and tests the existing cohort validator for the selected qualified build;
  `M3-EVID-005` records the formal results without rewriting Beta.1 or pooling incompatible builds.
- [ ] At least 8 cumulative target users complete at least 20 supported flows, including both
  Packs, three non-academic scenario families, and one mixed-Application recovery flow.
- [ ] At least 80% of supported attempts complete without maintainer operation; audited claim
  traceability and measured backup/restore success are 100%, unsupported claims are zero, and
  every participant understands that Exported is not Submitted.
...
**Entry:** The selected CLI-first Beta is qualified, feature freeze is active, and the formal
exact-build cohort in M4 is complete. LF-C10 must update and test the current evidence validator
before the new build can satisfy `M3-EVID-005`; Beta.1 history is preserved rather than relabelled.
...
Alpha.7 used the same future-tag flow for the breaking v4 checkpoint. Alpha.8 and Alpha.9 repeated
it, and exact public Alpha.10 is now the Codex-qualified Beta-entry checkpoint.

Before Beta.1, against exact qualified Alpha.10, use the dry-run-first
`canisend.beta-readiness/v2` writer with body-free maintainer validation. It binds the required
Codex/provider/contract evidence and records zero users; the cohort starts on public Beta.1 and is
required before RC.1. After `M4-READY-001` is Verified, run:
...
| Start Beta transition | M2 + M3 complete against Alpha.10; lean readiness refreshed within 24 hours; required Codex evidence passes; no applicable P0 blocker |
| Activate freeze | Beta.1 published, reverified, and recorded as qualified |
| Start RC.1 | Qualified Beta, active freeze, and post-Beta cohort evidence satisfy the M4 exit gate |
...
| Real-provider host coverage | Exact Alpha.10 Codex CLI for both Packs; Claude observations remain non-blocking | Required external Codex remains usable; qualify the selected CLI-first Host/build, with embedded expansion deferred |
| CLI-first user evidence | Not part of historical Beta.1 entry | Both Packs and changed journeys on the exact qualified CLI-first build; separate denominator and tested LF-C10 linkage |
| Cumulative target users | Zero claimed at historical Beta.1 entry | At least 8, target 8–12, on the validator-approved CLI-first build |
| Completed supported end-to-end flows | Zero claimed at historical Beta.1 entry | At least 20 on the validator-approved CLI-first build |
| Unassisted supported-flow completion | Not a historical Beta-entry gate | At least 80% on the exact CLI-first build; no pooled old-UI denominator or unresolved blocker pattern |
...
| Users who understand Exported is not Submitted | Not a Beta-entry gate; cohort starts on public Beta.1 | 100% |
...
10. **Completed 2026-08-29:** exact public Alpha.10, protected source
    `cd40180f2ff8ac957276f1948ba88da428511a82`, same-byte candidate/public evidence, required
    Academic and Generic Codex scenarios, and `canisend.provider-dogfood/v2` agree.
11. **Completed 2026-08-29:** qualified `canisend.beta-readiness/v2` against exact Alpha.10,
    both Codex Pack scenarios, current host/Pack/Skill digests, body-free maintainer review, and
    nine applicable blocker classes while recording zero users.
12. **Completed 2026-08-29:** `canisend.beta-contract-freeze/v2` binds the exact Alpha.10 v4
    contract, schema, migration, operation, exit-code, and package-layout surface.
13. **Completed 2026-08-29:** applied the reviewed Beta.1 source-stage transaction with matching
    preview/write reports and unchanged Alpha.10 authorities.
14. **Completed 2026-08-30:** built exact Beta.1 once, promoted the reviewed bytes without
    recompilation, published the prerelease, and independently reverified all 20 public assets.
15. **Completed 2026-08-30:** independently reverified public Beta.1, matched dry-run/write
    reports, and recorded the exact tag/source/run/signing targets in the qualification ledger.
16. **Completed 2026-08-30:** generated exact Beta.1 Homebrew, Scoop, and WinGet local candidates
    from independently verified public assets without modifying an external package index.
17. **Completed 2026-08-30:** activated feature freeze at exact protected repository baseline
    `acf25dc483643ca9be0210320775708da116b715` while retaining the qualified Beta source and zero
    initial exceptions.
18. **Completed 2026-09-03:** merged `M4-BETA2-001` and prepared the private Beta.2 source through
    `M4-BETA2-002` while preserving Beta.1 history and creating no publication side effect.
19. **Completed 2026-09-04:** built and independently inspected one exact nonpublishing Beta.2
    native candidate through `M4-BETA2-003` without creating a tag, Release, package, or
    qualification record.
20. **Source complete locally, 2026-09-04:** R0 authority/clean-v4 and R1 App Server lifecycle;
    inspect their protected integration separately. Keep `M4-BETA2-001/002` source preparation under
    the existing exact freeze protocol; no Beta.2 publication is implied.
21. **Completed 2026-09-06:** PR #226 merged previous work and closed LF-C01; App-first priority
    is superseded and private Beta.2 records are preserved.
22. **Current, 2026-09-15:** LF-C01–08 source integration and LF-C10/11 local automation are complete
    within the limits in §3.3. Execute first-use permission guidance, then collect the remaining
    LF-C05/07 actual Host acceptance and LF-C10/11 exact native evidence; do not repeat completed
    source slices or count simulated responses as human acceptance.
23. LF-C10 qualifies the standalone baseline and any tested exact-build cohort transition.
    LF-C06–08 add local coordination and conflict recovery; LF-C11 separately qualifies collaboration.
    LF-C09 extra Hosts are optional; LF-C12 cross-device work and embedded R2-R6 are deferred.
24. Qualify two distinct RCs only after applicable artifact and user gates pass; publish Stable only
    with explicit authorization. Paused native/GUI checks are not waived, and the formal cohort
    still requires at least 8 real users and 20 supported exact-build flows. Development approval
    does not authorize qualification writes or publication.
...
- [Embedded App checklist](../../../.trellis/tasks/09-04-embedded-agent-app-roadmap/implement.md)
  and [R2 execution plan](../../../.trellis/tasks/09-04-embedded-agent-app-r2/implement.md): R0/R1
  source complete; partial R2 and unfinished R2-R6 are deferred. The parent checklist now records
  CLI-first closeout/LF-C01. Retained paths require no Trellis runtime.
...
- [x] Exact public Alpha.10 is Codex-qualified for both built-in Packs and is the Beta.1 entry
  baseline; same-byte publication and body-free provider evidence agree.
...
- [x] Required Codex CLI Academic and Generic scenarios pass on exact Alpha.10 without mutation or
  submission; Claude and bounded MCP observations remain accurately classified.
- [x] Lean Beta.1 readiness binds exact Alpha.10, provider-dogfood v2, host/Pack/Skill digests,
  body-free maintainer review, and applicable blocker labels while claiming zero users.
- [x] The Beta.1 v2 freeze binds exact Alpha.10 Agent/Workspace/resource/Pack/Skill, schema,
  migration, operation, exit-code, and package-layout authority.
- [ ] Post-Beta mixed-Application user evidence meets the M4 pre-RC thresholds.
- [x] Beta.1 is qualified and establishes the exact feature-freeze baseline.
- [x] Private Beta.2 source preparation preserves Beta.1 history, keeps freeze active, and has no
  publication side effect.
- [x] One exact private Beta.2 native candidate passes independent integrity and provenance checks
  without publication or qualification.
- [ ] CLI-first baseline and applicable local collaboration scope are qualified on exact bytes.
- [ ] The formal M4 cohort has tested exact-build machine linkage; Beta.1 history is not relabelled
  and completing two full cohorts is not a release prerequisite.
