...
unchanged. The compiled inventory is now 28 CLI, 122 Tauri, and 27 MCP leaves, with 29 shared
operations and 82 qualified bindings in the two-Pack semantic matrix. Requirement extraction and
safe standalone CLI mutation approval remain follow-up work; the CLI does not falsely advertise
the in-memory approval-token pairs.
