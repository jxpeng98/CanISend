...
| Post-Alpha.7 source | Contains evidence-gate and usability follow-up work; no later commit or PR is published Alpha.7 bytes and every replacement checkpoint requires exact qualification |

The v4 application facade is shared by the desktop, CLI, and MCP surfaces, and exact Alpha.7 bytes
have been publicly reverified. Remaining M3 work is invited-user evidence, post-publication blocker
disposition, and Beta-entry readiness against one exact tested checkpoint. It must not create a
parallel engine, a second approval system, or relabel later source as published Alpha.7.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Why it blocks progress |
|---|---|---|
| Post-publication bootstrap usability fixes are green in Draft PR #174 but are not published Alpha.7 bytes | P0 before cohort entry | The approved disposition is to qualify exact Alpha.8 as the replacement cohort checkpoint before invited testing |
...
qualification/evidence child beside it. The current delivery task is `M3-ALPHA8-001`; resume
`M3-EVID-005` / Issue #70 after exact Alpha.8 qualification, and create `M4-READY-001` / Issue #71
only after the M3 exit gate passes. When sources disagree, correct the lower-authority projection
before advancing the affected gate.
...
### 11.6 Slice E — Alpha.7/Alpha.8 qualification and user cohorts
...
|---|---:|---|
| Alpha.8 invited cohort | 5–8 users and at least 10 complete flows across both Packs | Find initialization, mixed-Application, intake, Skills, consent, recovery, and export blockers |
...
- [ ] Exact public Alpha.8 binds the approved bootstrap fixes and repaired sequential-Alpha/Beta
...
7. Merge the bounded bootstrap fixes, repair sequential-Alpha/Beta release authority, and qualify
   exact `v1.0.0-alpha.8` as `M3-ALPHA8-001`.
8. Run the invited Alpha.8 cohort and body-free evidence. Only P0/P1 observed blockers may change
   the candidate line; defer feature requests.
9. Refresh Beta readiness from qualified Alpha.8, publish Beta.1, and activate feature freeze.
10. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
