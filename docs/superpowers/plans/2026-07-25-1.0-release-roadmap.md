...
|---|---|---|
| Previous-stage integration | First | R0/R1 source complete; partial R2 unaccepted. Reconcile the feature branch, latest main/Beta.2 records, and PR #225 through review and protected CI |
| CLI build and delivery independence | LF-C01–03 | CLI is the explicit default; five target dependency graphs have no GUI dependency. Standalone delivery preflight and package qualification remain LF-C03/10 work |
...
| LF-C02 | LF-C01 | Explicit CLI default build/test selection without frontend prerequisites; preserve command behavior | Source implemented; protected integration tracked in the existing plan |
| LF-C03 | LF-C02 | Verify standalone resources, install/upgrade/repair/uninstall without an App, source checkout or consumer language runtime | Planned |
...
LF-C01 merged through PR #226. LF-C02 implements default CLI selection and reuses existing core CI;
its protected integration is recorded in the existing plan and PR. LF-C03 is the next bounded slice. LF-C10 does not wait for
collaboration or extra Hosts. No later LF-C task is implemented by accepting this roadmap.
...
21. **Current:** reconcile and review the latest work branch with main, close the App-first stage
    as superseded, and finish LF-C01's dependency/function audit. Preserve private Beta.2 records.
22. **Next bounded implementation:** LF-C02 CLI build/test selection; then LF-C03 resources and
    standalone installation, LF-C04 trusted headless approval, and LF-C05 one Host plus resumption.
