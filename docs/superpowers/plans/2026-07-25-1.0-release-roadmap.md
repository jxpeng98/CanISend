...
single-package npm testing prerelease with simplified Agent instructions and Skills. Beta.5
was published through OIDC in `release.yml` run `34285066117`; prepare and verify the next
CLI before publishing to `next`, declaring only the supplied native platforms.
