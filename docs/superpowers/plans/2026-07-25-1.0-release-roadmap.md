...
checkpoint at `beta-qualifying`, local package-channel candidates are complete, and active feature
freeze is next
...
**Next intended checkpoint:** `v1.0.0-beta.1` consented, body-free mixed-Application invited cohort
under `M3-EVID-005` / Issue #70. The published Beta artifact source remains
`6e1397b79031cad54e794ccdc9edca2153f23b3e`; feature freeze is active at separate protected
repository baseline `acf25dc483643ca9be0210320775708da116b715`. The cohort remains required
before RC.1 planning.
...
candidate, qualification ledger, and local package-channel candidates are complete. Active freeze
is next; invited-user evidence runs on public Beta.1 before RC.1. None of these tasks may create a
...
|---|---|---|
| Beta.1 active freeze is pending in Issue #77 | P0 before RC | Qualified package metadata does not activate the source baseline |
...
metadata and protected reconciliation. `M4-FREEZE-002` records the exact protected repository
baseline while retaining the qualified Beta artifact source; `M3-EVID-005` / Issue #70 is the next
task after protected activation reconciliation and remains required before RC.1. When sources
...
17. **Completed 2026-08-30:** activated feature freeze at exact protected repository baseline
    ...
    initial exceptions.
18. **Next:** complete `M3-EVID-005` / Issue #70 on public Beta.1 before RC.1 planning.
19. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
