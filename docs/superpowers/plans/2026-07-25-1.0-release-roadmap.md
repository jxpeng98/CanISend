...
checkpoint at `beta-qualifying`, feature freeze is active, and private Beta.2 source readiness is
the current release-engineering checkpoint
...
**Reconciled for private Beta.2 source preparation:** 2026-09-03
...
**Next intended checkpoint:** `v1.0.0-beta.1` remains the qualified public checkpoint while the
private Beta.2 source is prepared under the same active freeze. The published Beta artifact source
remains `6e1397b79031cad54e794ccdc9edca2153f23b3e`; feature freeze is active at separate protected
repository baseline `acf25dc483643ca9be0210320775708da116b715`. No Beta.2 tag, release, package,
or qualification is authorized here, and the consented body-free cohort remains required before
RC.1 planning. Any future validation-build change requires an explicit tested evidence-contract transition
under LF-C10; the current Beta.1 validator is unchanged by this document.
...
Private Beta.2 readiness now proves the supported CLI, Skills, MCP, and Workspace journey before
its source transition; invited-user evidence remains required before RC.1. None of these tasks may create a
parallel engine, a second approval system, modify an external package index, or rewrite historical
Alpha evidence.
...
`M4-BETA2-001` and `M4-BETA2-002` separate governance/readiness from the controlled source
transition; neither task authorizes Beta.2 qualification or publication.
...
- [x] New feature requests are assigned to 1.1 unless a documented release exception resets the
  affected evidence.
- [ ] Exact sequential-Beta policy and the App-closed CLI/Skills/MCP/Workspace gate are merged.
- [ ] Private Beta.2 source is prepared with Beta.1 preserved in history and no publication side effect.
...
17. **Completed 2026-08-30:** activated feature freeze at exact protected repository baseline
    ...
    initial exceptions.
18. **Source complete locally, 2026-09-04:** R0 authority/clean-v4 and R1 App Server lifecycle;
...
19. **Current:** reconcile and review the latest work branch with main, close the App-first stage
    as superseded, and finish LF-C01's dependency/function audit. Preserve private Beta.2 records.
20. **Next bounded implementation:** LF-C02 CLI build/test selection; then LF-C03 resources and
    standalone installation, LF-C04 trusted headless approval, and LF-C05 one Host plus resumption.
21. LF-C10 qualifies the standalone baseline and any tested exact-build cohort transition.
...
22. Qualify two distinct RCs only after applicable artifact and user gates pass; publish Stable only
...
- [ ] Private Beta.2 source preparation preserves Beta.1 history, keeps freeze active, and has no
  publication side effect.
- [ ] The approved embedded R0-R4 scope is qualified on exact new Beta bytes, with effective private
  tool isolation, explicit consent, durable history/origin, complete Workbench, and user evidence.
