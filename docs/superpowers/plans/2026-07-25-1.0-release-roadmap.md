...
| Alpha.6 publication evidence needs one reconciled record | P0 before Workspace v4 implementation | Public runs and M2 Milestone state exist, but Issue bodies did not consistently retain evidence and no exact-candidate real-provider record survives; the gap must be explicit rather than inferred from labels |
...
path so native qualification covers it with the App absent. M3-CLI-001 remains in progress for the
remaining neutral data/association operations and five-target release qualification.
...
unchanged. The compiled inventory is now 28 CLI, 122 Tauri, and 29 MCP leaves, with 31 shared
operations and 84 qualified bindings in the two-Pack semantic matrix. Exact Source-bound
Requirement extraction now appends reviewed, deduplicated proposals through its own guarded
preview/commit pair; safe standalone CLI mutation approval remains follow-up work, and the CLI does
not falsely advertise the in-memory approval-token pairs.
...
visible workflow and callable desktop authority share the same approval boundary. Local frontend,
accessibility, Rust contract, source-gate, and protected-branch evidence remain required before
M3-DESKTOP-003 becomes Verified.
