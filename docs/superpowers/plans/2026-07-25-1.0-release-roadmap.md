...
| Post-Alpha.8 source | Closes the Store→IO production edge under M3-ARCH-001; no later commit or PR is published Alpha.8 bytes, so the changed source requires exact Alpha.9 qualification |
...
bounded MCP host. M3-ARCH-001 changes later source bytes, so Alpha.9 qualification now precedes
...
| [`M3-ALPHA9-001`](https://github.com/jxpeng98/CanISend/issues/183): changed architecture bytes are not yet a qualified checkpoint | P0 before cohort evidence resumes | Alpha.9 must bind the exact protected source, both Packs, provider/host evidence, and affected render/recovery scenarios without rewriting Alpha.8 |
...
qualification/evidence child beside it. `M3-ARCH-001` is the current architecture task while
`M3-EVID-005` / Issue #70 waits for real invited users. Its changed product bytes make
`M3-ALPHA9-001` the next checkpoint task before affected cohort evidence can resume. Create
...
8. Integrate the Store→IO production-edge removal under `M3-ARCH-001` through protected CI; after
   merge, qualify the next sequential Alpha before affected cohort evidence resumes.
9. Run the invited cohort and body-free evidence on the latest exact qualified Alpha. Only P0/P1
   observed blockers may change the candidate line; defer feature requests.
10. Refresh Beta readiness from that qualified Alpha, publish Beta.1, and activate feature freeze.
11. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
