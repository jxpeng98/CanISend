...
**Status:** Active — authoritative; Alpha.10 is the Codex-qualified public checkpoint and Beta.1
readiness is next
...
contract after lean readiness confirms the exact checkpoint and no applicable P0 blocker. The
invited cohort runs on Beta.1 and remains required before RC. Beta is not yet authorized.
...
The next task is lean Beta.1 readiness; invited-user evidence runs on public Beta.1 before RC.1.
Neither task may create a parallel engine, a second approval system, or rewrite historical Alpha
evidence.
...
|---|---|---|
| Lean Alpha.10-bound Beta readiness is pending in Issue #71 | P0 before Beta | Beta preparation must bind exact source, Codex evidence, contracts, Pack/Skill digests, maintainer validation, and applicable blocker classes without inventing user evidence |
...
Codex-first qualification evidence. `M4-READY-001` / Issue #71 is next. `M3-EVID-005` / Issue #70
is rebound to Alpha.10 but runs after public Beta.1 and before RC.1. When sources disagree,
...
11. **Next:** complete lean Alpha.10-bound readiness, publish one build-once Beta.1 candidate,
    independently reverify it, and activate feature freeze.
12. Run the invited cohort on public Beta.1 and complete `M3-EVID-005` / Issue #70 before RC.1.
13. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
