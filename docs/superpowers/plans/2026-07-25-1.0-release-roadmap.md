...
**Status:** Active — authoritative; M3 Workspace v4 and Agent v4 rebase is the current milestone
...
`1.0.0-alpha.6`. This is the current `main` checkpoint after the published Alpha.6 fixes; changed
bytes still require a new exact candidate before they may be described as Alpha.7.

**Current public checkpoint:** [`v1.0.0-alpha.6`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-alpha.6)
at `e524a6d2168c5b71525149e575861c79548aaf13`, published 2026-08-04. The tag, public artifacts,
...
**Next intended checkpoint:** `v1.0.0-alpha.7` is a breaking framework checkpoint. It introduces a
neutral multi-Application Workspace v4, Application-level Pack binding, connected generic intake,
App-led initialization, and a newly designed Agent v4/Skills workflow. It carries no promise to
run Alpha.6 or earlier Skills, Agent requests, job aliases, or Workspace formats. Alpha.7 is the
only Alpha eligible to become the Beta contract baseline.
...
| Codex and Claude Code | Supported external hosts through new v4 Skills | Real-provider setup/new/resume/approval/failure-recovery dogfood |
...
| Post-Alpha.6 `main` | Contains Alpha.6 follow-up fixes; Alpha.7 architecture work has not yet been qualified as a public release |

The application facade already exists in `canisend-app`, but the desktop and Workspace ownership
model still expose academic/generic modes and disconnected intake. Alpha.7 must evolve that one
facade into the v4 model; it must not create a parallel engine beside either Pack.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Why it blocks progress |
|---|---|---|
| One Workspace cannot yet host academic and generic Applications as equal peers | P0 before Alpha.7 | Workspace-level mode and Pack assumptions contradict the v4 product boundary |
| Generic intake is still disconnected manual entry | P0 before Alpha.7 | Sources, Requirements, Evidence, and Applications do not yet form one traceable flow |
| Desktop initialization does not create the complete headless host setup | P0 before Alpha.7 | A user cannot initialize once in the App and continue confidently through CLI, Codex, or Claude Code |
| Current Skills and Agent protocol model the compatibility-era product | P0 before Alpha.7 | The new workflow needs canonical v4 tasks and schemas rather than additional legacy negotiation |
| The desktop drag region lacks the required Tauri ACL permission | P0 before Alpha.7 | A public App workflow produces `window|start_dragging not allowed by ACL` |
| Alpha.6 publication projections are not reconciled | P0 before Alpha.7 candidate | The local Beta-readiness record still says `pending-alpha-publication` and has null source/run fields despite the public tag; a later release must not inherit contradictory authority |
...
- Alpha.7 candidate work starts only after clean-v4 App, CLI, MCP, Codex, and Claude Code flows pass.
- M3 multi-Application real-user evidence starts only from a published Alpha.7.
...
flowchart LR
    ...
    M1A --> M2
    M1B --> M2
    M2 --> M3["M3: Workspace v4, Agent v4, and Alpha.7 validation"]
...
| M3 — Workspace v4, new Skills/Agent workflow, Alpha.7, and validation | 5–8 implementation weeks plus 2–4 validation weeks | M2 | Public breaking Alpha.7, body-free validation evidence, closed blockers |
...
## 11. M3 — Workspace v4, new Agent workflow, Alpha.7, and user validation
...
on earlier Skills or Agent protocols.

**Entry:** Public Alpha.6 exists and establishes the historical v3 checkpoint. M3 design and
implementation may begin, but M3-TRUTH-001 and every unresolved M2 gate must close before an
Alpha.7 candidate. Tests use clean disposable v4 Workspaces and synthetic or explicitly consented
materials. The change is allowed to break Alpha.6 development contracts before Beta. Alpha.6
tag-scoped facts remain immutable.

**GitHub execution ledger:** [Alpha.7 Milestone #4](https://github.com/jxpeng98/CanISend/milestone/4)
contains one canonical Issue per task below. Architecture and model work is tracked by
...
| M3-BOOTSTRAP-001 | P0 | M3-STORE-001, M3-SURFACE-001 | Implement App-led v4 initialization: choose external path, create Workspace, capture minimum basic Profile data, validate Packs, and preview/install new Skills plus MCP configuration | A clean supported Mac completes setup without terminal use; cancel and failure leave no partial Workspace, host file, registry entry, or private body |
...
| M3-CLI-001 | P0 | M3-SURFACE-001, M3-AGENT-DESIGN-001 | Make clean-v4 initialization, host setup/status, basic data management, Application operations, MCP stdio, and recovery available through the standalone CLI | Five-target tests run with the App absent; CLI and MCP receipts match in-process App semantics and never write authority outside the application facade |
...
| M3-AGENT-001 | P0 | M3-AGENT-RESOURCES-001 | Complete new and resumed Codex sessions for both Packs in one Workspace on exact Alpha.7 bytes | Discovery, context selection, MCP-first operation, native CLI headless equivalent, approval, cancel, commit, reopen, and stale recovery are recorded |
| M3-AGENT-002 | P0 | M3-AGENT-RESOURCES-001 | Complete the equivalent Claude Code scenarios | The same body-free scenario matrix passes with no host-specific business rule or direct `.canisend` write |
...
resources. Codex and Claude Code can continue work after the App closes, and changes are visible
when the App reopens.

### 11.6 Slice E — Alpha.7 qualification and user cohorts
...
|---|---:|---|
| Alpha.7 invited cohort | 5–8 users and at least 10 complete flows across both Packs | Find initialization, mixed-Application, intake, Skills, consent, recovery, and export blockers |
...
- new Agent v4 Skills in Codex and Claude Code, plus one schema-conforming MCP client;
...
- [ ] Exact public Alpha.7 binds Workspace v4, Agent v4, new Skills, both built-in Packs, and all
...
- [ ] App, CLI, MCP, Codex, and Claude Code pass one v4 semantic suite; App-closed operation and
  reopen reconciliation pass.
- [ ] Old Skills, Agent v2/v3, job aliases, and Workspace v2/v3 are absent from the supported
  Alpha.7 matrix and fail closed before mutation when detected.
...
| ID | Priority | Deliverable | Verification |
|---|---|---|---|
| M4-READY-001 | P0 | Refresh Beta readiness against Alpha.7 and reviewed mixed-Application Agent/user evidence | Snapshot is no older than 24 hours, binds Workspace v4, Agent v4, Skills and both Pack digests, is body-free, and zero issues is not the only blocker evidence |
| M4-FREEZE-001 | P0 | Run `release freeze-candidate` for Agent v4, Workspace v4, Skills manifest/resources, Pack v1, both built-in Packs, schemas, operation registry, exit codes, and bundle layout | Baseline digests match the qualified Alpha.7 surface; no Alpha.6, v2, or v3-only baseline can pass |
...
| M5-UPGRADE-001 | P0 | Run clean-v4 Alpha.7→Beta.1→RC.1 five-target product/resource upgrade, Pack-update, rollback, and restore-to-new-path qualification | `record-upgrade-qualification` accepts the exact v4 evidence chain, both Pack identities, and Skills/resource digests; no legacy Workspace migration is claimed |
...
| Real-provider host coverage | New Agent v4 Skills in Codex and Claude Code for both Packs | No regression on final candidate |
...
1. Reconcile Alpha.6 tag/source/run/public-verification projections and close or explicitly block
   every remaining M2 gate (`M3-TRUTH-001`).
2. Freeze ADR-RN-0020 and the clean-v4 non-goals; remove old-Skills compatibility from active
   plans, Milestones, Issues, and release acceptance.
3. Implement Workspace v4, Application-level Pack ownership, explicit shared-data associations,
   clean-v4 recovery, and unsupported-legacy no-mutation (`M3-WORKSPACE/MODEL/ASSOC/STORE/LEGACY`).
4. Rebind both Packs, implement connected intake, and freeze the canonical v4 operation surface
   plus cross-adapter semantic matrix (`M3-PACK/INTAKE/SURFACE/PARITY`).
5. Fix the Tauri drag ACL, then implement App-led Workspace/basic-data/Skills/MCP initialization
   and the mixed-Application desktop collection (`M3-DESKTOP/BOOTSTRAP`).
6. Define Agent v4 and its canonical task-resource model; implement headless CLI/MCP parity
   without App dependency (`M3-AGENT-DESIGN`, `M3-CLI`).
7. Generate new Codex and Claude Code Skills, host setup/status/uninstall flows, integrity checks,
   and unsupported-old-resource refusal (`M3-AGENT-RESOURCES`, `M3-AGENT-004`).
8. Run the same academic and generic workflows through App, CLI, MCP, Codex, and Claude Code;
   include App-closed continuation, reopen reconciliation, cross-Application denial, and failures.
9. Freeze Alpha.7 scope only when every M3 P0 source task passes; build once, qualify all targets,
   promote exact bytes, and publicly reverify (`M3-ALPHA7-001`).
10. Run both user cohorts and body-free provider/user evidence against exact Alpha.7.
11. Refresh Beta readiness from Alpha.7 and qualify Beta.1 against only v4 contracts.
12. Freeze, qualify two RCs, complete every Stable evidence class, and publish Stable only after
...
- [ ] Alpha.6 publication projections and every M2 evidence gate are reconciled without rewriting
  historical facts.
- [ ] Alpha.7 truthfully qualifies Workspace v4, Application-level Pack binding, new Agent v4
...
- [ ] Real Codex, Claude Code, MCP-client, and mixed-Application user evidence meets M3 thresholds.
