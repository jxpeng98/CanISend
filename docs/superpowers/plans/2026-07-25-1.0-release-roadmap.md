...
flowchart LR
    ...
    M1A --> M2
    M1B --> M2
    M2 --> M3["M3: dual-pack hardening and Alpha.7 validation"]
...
| M3 — Dual-pack hardening, Alpha.7, Agent and user validation | 3–6 implementation weeks plus 2–4 validation weeks | M2 | Public hardened Alpha.7, body-free validation evidence, closed blockers |
...
- [ ] Pack v1, Agent v3, Workspace v3, both built-in Pack digests, and v2 compatibility are
...
- [ ] Structural and semantic parity pass through both workflow Packs and the neutral kernel.
- [ ] Backup/restore and PDF preview/export use the expected exact artifact bytes.
- [ ] Codex and Claude candidate dogfood passes for both Packs on the exact candidate bytes.
- [ ] Public documentation describes Alpha.6 as a dual-Pack migration checkpoint, explains the
...
## 11. M3 — Dual-pack hardening, Alpha.7, and user validation

**Objective:** Harden the two Packs shipped in Alpha.6, prove that CanISend is one generic kernel
exercised by materially different Packs, and replace synthetic-only confidence with consented
Agent and user evidence on exact Alpha.7 bytes.
...
| ID | Priority | Deliverable | Verification |
|---|---|---|---|
| M3-PACK-001 | P0 | Harden `org.canisend.generic-application` from Alpha.6 feedback while preserving configurable Opportunity metadata, Requirement/Evidence categories, stage labels, and Deliverable kinds | The manifest contains no academic-only required field; a fixture completes with at least two custom Deliverable kinds |
| M3-PACK-002 | P0 | Extend and qualify the shipped synthetic professional-job, grant/fellowship, admission, tender/proposal, and internal-dossier examples | Every example validates offline, is body-safe, and at least three materially different examples complete the end-to-end suite |
| M3-SURFACE-001 | P0 | Stabilize `application` and `opportunity` as the canonical CLI/MCP/desktop/Agent v3 surfaces with Pack selection and configuration | New documentation uses neutral operations; v2/job aliases remain bounded to the academic pack |
...
After Alpha.6 migration and Generic Pack feedback closes and M3's dual-Pack hardening and source
gates pass, repeat the same future-tag flow for `v1.0.0-alpha.7` with a distinct candidate epoch.
Alpha.7 must be publicly downloaded and reverified before real-provider/user validation can
authorize Beta.
...
| Start Alpha.7 candidate | Alpha.6 migration and Generic Pack feedback closed; dual-Pack hardening and source parity pass |
