...
**Status:** Active — authoritative; Beta.1 is the current build-once, publicly reverified
prerelease at `beta-qualifying`, and exact qualification recording is next
...
**Current public checkpoint:** [`v1.0.0-alpha.10`](https://github.com/jxpeng98/CanISend/releases/tag/v1.0.0-alpha.10)
at `cd40180f2ff8ac957276f1948ba88da428511a82`, published 2026-08-29. It remains the immutable
Codex-qualified entry checkpoint.
...
**Current machine stage:** Beta / `beta-qualifying`; Beta.1 is public and independently verified,
while qualification, package-channel candidates, feature-freeze activation, and invited-user
evidence remain pending. No RC is recorded and Stable is not authorized.
...
**Next intended checkpoint:** `v1.0.0-beta.1` qualification is recorded from the independently
downloaded public assets, followed by local package-channel candidates and feature-freeze
activation against the qualified source. The invited cohort then runs on public Beta.1 and remains
required before RC. Beta.1 is public but not yet qualified or frozen.
...
Lean Beta.1 readiness and its exact v4 contract freeze are qualified; the source-stage transition
and build-once public candidate are complete, while ledger qualification is next. Invited-user
evidence runs on public Beta.1 before RC.1. None of these tasks may create a parallel engine, a
second approval system, or rewrite historical Alpha evidence.

### 3.2 Open gaps that govern the critical path

| Gap | Priority | Why it blocks progress |
|---|---|---|
| Beta.1 qualification is pending in Issue #75 | P0 before freeze | Publicly verified assets do not establish qualification until the ledger binds the exact tag, source, run, and manifest |
...
`M4-CANDIDATE-001` / Issue #74 is Verified by exact public Beta.1 and this protected
reconciliation; `M4-LEDGER-001` / Issue #75 is next. `M3-EVID-005` / Issue #70 runs after public
Beta.1 and before RC.1. When sources disagree,
...
records the exact boundary; qualification and feature freeze remain pending.
...
15. **Next:** record Beta qualification, generate package-channel candidates, activate feature
    freeze, then complete `M3-EVID-005` / Issue #70 before RC.1.
16. Qualify two distinct RCs, complete every Stable evidence class, and publish Stable only after
