...
- Ask first: before reading full private CVs, statements, full job adverts, references, PDFs, source URLs, Evidence
  snapshots/candidates/catalogs, generated packages, `criteria.json`, `criterion_matches.json`,
  `application_brief.yaml`, or `required_document_plan.json`; also ask before enabling LLM-backed CLI flags/providers.
  Criteria may contain corrected wording; Match, Brief, and the document plan remain Tier 2.
...
- Resume the deterministic Decision Spine through `stage run --stage evidence|parse|confirm|match|brief`; Evidence,
  Match, and Brief planning require no platform API, network, or configured provider. Never write their run files or
  authoritative outputs directly.
...
- Use `corrections status|init|update`, `decision status|init|update`, `brief status|init|update`, and
  `user-mutation recover` for user-owned writes. Never directly overwrite or normalize any user YAML; use one strict
  patch, current revision/hash, and explicit
...
- User YAML/private mutation candidates/corrected Criteria are Tier 2; Tier 1 receipts and AgentResponse never contain
  correction text, rationale, Brief values, or document source text. CAS assumes a stable job directory and
  cooperative writers, so avoid concurrent
  manual saves.
